...
### What Was Done
- Read all 14 migrations (001â€“014) and all 18 route files
...
### Key Decisions
- Documented `cv_batch_stage_recipes.recipe_id` as a missing FK (comment in 003 says 006 would add it, but 006 doesn't). Flagged but not fixed â€” that's a schema change requiring a separate migration.
...
- The migration content in the doc includes PRAGMA changes in `up()` but notes they only affect the migration-time connection â€” companion change to `initDB()` is required.

### Files Modified/Created
- `docs/audit-schema-performance.md` â€” new file (primary deliverable)
- `.claude/session_context.md` â€” appended this entry

### Notes for Next Tasks
- Migration `015_indexes.ts` is ready to implement from the doc â€” content is in Section 6
- `initDB()` in `src/db/index.ts` needs 5 additional PRAGMA lines (synchronous, busy_timeout, cache_size, temp_store, mmap_size)
- `GET /harvest/waste-trim` needs a LIMIT added â€” currently unbounded
...
### Key Decisions
- Used `requestAnimationFrame` loop with `jsQR` on canvas frames â€” no external scanning SDK needed, works in all modern mobile browsers.
- Print window uses inline CSS `@page { size: letter }` + Google Fonts import for JetBrains Mono â€” no server-side PDF generation needed.
- `ContainerScanner` is `position: fixed; inset: 0` so it overlays the full viewport including the NavBar â€” correct for a full-screen scanner UX.
...
- The scanner navigates to `/containers/:containerId` on successful scan â€” that route already exists (ContainerDetail.jsx).
- `api.getContainers()` already supports `sub_zone_id` as a query param â€” ContainerLabels uses it for filtering.
- Build output: ~777KB gzipped to 190KB â€” chunk size warning is pre-existing, not caused by this change.
...
- Created `src/tests/domain-utils.test.ts` â€” 27 tests covering all four exported functions plus METRC UID and container ID regex validation
- Created `client/src/tests/api.test.js` â€” 30 smoke tests asserting all api methods exist

### Key Decisions
- `makeBatchName` and `makeHarvestBatchName` are new exports; the batch name logic was previously inline in `enrichBatch()`
- Root vitest picks up both `src/tests/` and `client/src/tests/` â€” 57 tests total run via `npm test` from project root; `cd client && npm test` runs the 30 frontend-only tests
- The `@vitejs/plugin-react` deprecation warnings (esbuild â†’ oxc) are cosmetic and do not affect test results
...
- `src/lib/domain-utils.ts` (new â€” shared domain functions)
- `src/tests/domain-utils.test.ts` (new â€” 27 unit tests)
- `client/src/tests/api.test.js` (new â€” 30 method-existence smoke tests)
...
- `src/lib/domain-utils.ts` is the home for pure domain functions shared across routes or needed in tests â€” add to it as METRC export, PHI/REI calculations, etc. are implemented
...
- Kept cross-field validations as code (foliar_recipe_id XOR input_id, rate_value required with input_id) â€” these are business rules, not type validation
...
- For ec_measured/ph_measured: used z.number() (not .positive()) â€” value 0 is valid for "meter-error" case
...
- Zod schemas are defined as module-level constants before the plugin â€” available for future test use
...
### What Was Done
- Created `src/api/routes/harvest.ts` â€” Fastify plugin at `/api/harvest`
...
### Key Decisions
- `cv_plant_assignments` uses `placed_at` / `placed_by` (not `assigned_at`) after migration 014 â€” used correctly in unassignment UPDATE
- `cv_container_state` uses `current_batch_id` (not `current_plant_batch_id`) â€” schema cross-checked before writing
...
- Container `from_state` is read from `cv_container_state` at time of final harvest rather than hardcoded as 'active' â€” handles edge cases
- GET /waste-trim uses spread `...values` for dynamic WHERE â€” valid better-sqlite3 pattern

### Files Modified/Created
- `src/api/routes/harvest.ts` (new â€” 290 lines)
...
- Harvest batch name format: "Strain | MM/DD/YYYY | HB|MB | Auto|Photo" â€” uses UTC date from started_at (America/Chicago conversion not yet implemented in utils)
- `metrc_name` is computed at creation and stored immutably; not re-computed on updates
- The `partial_harvest` event type is allowed at any batch status where a manicure batch exists (task spec says batch must be 'harvesting' â€” enforced via harvest_batch.status='in_progress' which requires batch to be 'harvesting' to have been created)
...
- Created `client/src/pages/harvest/` directory with 5 new pages:
  - `HarvestDashboard.jsx` â€” route `/harvest/:batchId`, shows harvest batches (HB/MB), plant list grouped by container, create HB/MB buttons (supervisor), waste trim link, and per-plant Partial/Final Harvest buttons
  - `PartialHarvestForm.jsx` â€” route `/harvest/:batchId/partial?...`, locked context + product_type chips + wet weight + notes; calls recordHarvestEvent with partial_harvest
  - `FinalHarvestForm.jsx` â€” route `/harvest/:batchId/final?...`, tag verification step FIRST (confirms physical tag matches last 4 of METRC tag), then product_type + weight + notes; red warning + calls recordHarvestEvent with final_harvest
  - `WasteTrimForm.jsx` â€” route `/harvest/waste-trim/new?batch_id=X`, trim_reason chips + weight; draft persistence under cv_draft_waste_trim; available at any batch status
  - `WeatherEventClose.jsx` â€” route `/harvest/batches/:harvestBatchId/force-close?batch_id=X`, supervisor-only; required close_notes (min 20 chars) + new conditions; calls forceCloseHarvestBatch
- Updated `client/src/pages/batches/BatchDetail.jsx`:
  ...
  - Added "Harvest Dashboard â†’" full-width green-800 button when status === 'harvesting'
...
- Updated `client/src/pages/containers/ContainerDetail.jsx`:
  ...
  - Added "Partial Harvest" / "Final Harvest" buttons for active/empty containers during harvesting â€” derives assignment_id and harvest_batch_ids from harvestCtx
...
- ContainerDetail loads harvestCtx lazily via useEffect triggered on data change â€” avoids adding getHarvestStatus to the main load path
- FinalHarvestForm: tag verification is a hard gate â€” form fields only appear after confirmation
- WeatherEventClose uses batch_id query param (not in the route path) to navigate back and to load harvest status
- React Router v6 static segments beat dynamic ones â€” waste-trim and batches routes correctly take precedence over :batchId
...
### Notes for Next Tasks
- The readiness summary endpoint shape isn't fully known â€” the harvest readiness section defensively checks for containers_assessed / total_containers / rows fields
- getReadinessSummary presumably returns { containers_assessed, total_containers, rows: [{ row_id, ready_count, total_count, pct_ready }] } â€” verify against the observations route when needed
- HarvestDashboard shows "No MB active" / "No HB active" placeholders when no in_progress batch of that type exists â€” user must create batches first via supervisor buttons
...
### What Was Done
- Created `src/api/routes/plant-loss.ts` â€” Fastify plugin at `/api/plant-loss` with:
  - `POST /` â€” record plant loss; validates batch/assignment/container; in one transaction: inserts cv_plant_loss_events, unassigns cv_plant_assignments, transitions container activeâ†’empty if no active assignments remain
  - `GET /` â€” list loss events with optional batch_id/container_id/metrc_sync_status filters
  - `POST /replacements` â€” assign replacement plant (untagged); transitions container emptyâ†’active in same transaction
...
- Created `client/src/pages/containers/PlantLossForm.jsx` â€” field-optimized form at `/containers/:containerId/loss`
- Created `client/src/pages/containers/PlantReplacementForm.jsx` â€” simple form at `/containers/:containerId/replacement`
...
- Replacement creates assignment with `metrc_plant_tag = NULL` â€” tag assigned separately via tag-assignment workflow
- Loss type â†’ unassign_reason: death_* â†’ 'died', physical_damage/removal_* â†’ 'destroyed', other/accidental â†’ 'other'
...
## Task: 6. Container lifecycle (teardown, soil sample, startup) â€” features 23-25
**Completed:** 2026-05-21

### What Was Done
- Created `src/api/routes/container-lifecycle.ts` â€” Fastify plugin registered at `/api/containers`:
  - `POST /:containerId/teardown` â€” validates state is 'active'/'empty' + batch_id matches; inserts teardown event + transitions to 'teardown'
  - `PATCH /:containerId/teardown/:teardownId` â€” partial update of checklist fields (dynamic SET clause)
  - `POST /:containerId/soil-samples` â€” creates sample, optionally links to teardown, updates cv_teardown_events.soil_sample_collected
  - `GET /:containerId/soil-samples` â€” returns samples with nested results array
  - `POST /:containerId/soil-samples/:sampleId/results` â€” accepts array or { results: [...] }; inserts result rows + marks results_received
  - `POST /:containerId/startup` â€” validates 'teardown' state; inserts startup event + transitions to 'startup' (clears current_batch_id)
  - `POST /:containerId/startup/:startupId/ready` â€” supervisor only; sign-off updates startup + transitions to 'ready'
...
- Created 4 new frontend pages:
  - `TeardownForm.jsx` â€” checklist UI with toggle buttons, draft persistence; navigates to SoilSampleForm if soil_sample_collected
  - `SoilSampleForm.jsx` â€” sample_label + sample_type chips + lab_name; resolves teardown_id='new' to most recent teardown from container data
  - `StartupForm.jsx` â€” pre-populates prior teardown/sample from loaded data; media_replaced_pct chips (33/50/100) + custom; draft persistence
  - `StartupReadyForm.jsx` â€” supervisor-only sign-off; shows startup summary; non-supervisors see info message
- Updated `ContainerDetail.jsx`:
  - Added `soilSamples` state + separate useEffect calling `api.getSoilSamples`
  - Lifecycle action buttons by state: 'active'/'empty' â†’ Begin Teardown; 'teardown' â†’ Log Soil Sample + Begin Startup; 'startup' â†’ Mark as Ready (supervisor)
...
### Files Modified/Created
- `src/api/routes/container-lifecycle.ts` (new â€” 6 endpoints)
- `src/api/app.ts` (import + register)
...
- SoilSampleForm handles `teardown_id=new` by looking at `teardown_events[0]` â€” relies on ContainerDetail having loaded the teardown event before navigating
- StartupForm pre-populates prior_teardown_id and prior_soil_sample_id but does NOT send them if null â€” backend accepts nullable optionals
- The `amendments_applied_count` on cv_startup_events is not auto-incremented when amendments are logged via container-amendments route â€” this is a known gap; would need a trigger or explicit count update

## Task: 7. Export reports (METRC, MDA, cultivation record) â€” features 11-13
**Completed:** 2026-05-21

### What Was Done
- Created `src/api/routes/exports.ts` â€” Fastify plugin at `/api/exports` with three routes:
  - `GET /metrc-additives` â€” unified METRC Record Additives export (all 4 application types, JSON + CSV)
  - `GET /mda-pesticide` â€” MDA pesticide report per MN Statute 18B.37 (JSON + CSV, date_from/to required)
  - `GET /cultivation-record/:batchId` â€” full per-batch compliance record (JSON) per MN Statute 342.25
...
- Created 3 frontend pages under `client/src/pages/exports/`:
  - `MetrcExport.jsx` â€” route `/exports/metrc`: filter by batch_id + date range, preview table, CSV download
  - `MdaReport.jsx` â€” route `/exports/mda-pesticide`: required date range, compliance note, preview table, CSV download
  - `CultivationRecord.jsx` â€” route `/exports/cultivation-record`: batch picker, summary stats, JSON download
- Added "Exports & Reports" section to `ApplicationsHub.jsx` with links to all three pages
- Registered all three routes in `App.jsx`

### Key Decisions
- Farmstock items are resolved via API calls (separate DB â€” Option B integration), not direct DB join. Product names fall back to `Input #N` when farmstock is not configured
...
- **CRITICAL:** `GET /api/auth/users` has no auth middleware â€” exposes user enumeration including roles to unauthenticated clients
...
- **HIGH:** `api.getItems` in api.js calls `/api/items` which doesn't exist (routes are at `/api/catalog/items`) â€” broken method
- **HIGH feature gaps:** Entire planting-plans feature (8 routes, owns `cult-hoop â†’ field-veg` transition) and most of tag-assignments (4 of 5 routes) have no frontend api.js methods â€” METRC tagging and field planting workflows unreachable from UI
- **MEDIUM:** Recipe POST bodies, strain mutations, and container state mutations lack Zod validation (cast to interface directly)
- All SQL uses parameterized queries â€” no injection risk found
...
### Files Modified/Created
- `docs/audit-api-security.md` (new â€” 319 lines, committed and pushed)
...
### Key Decisions (findings summary)
- **CRITICAL:** No REI pre-entry check in `ContainerDetail.jsx` or `ContainerScanner.jsx` â€” a QR scan into an active-REI area goes straight to the container record with no full-screen warning (violates Rule 12)
- **CRITICAL:** `Today.jsx` silently swallows all API load errors â€” failed loads look identical to empty states
...
- **CRITICAL:** `PesticideNew.jsx` has no offline/network-error handling â€” a failed save during spotty WiFi shows a generic error rather than queuing for sync (all other application forms handle this correctly)
...
- NavBar touch targets are ~40px â€” below the 56pt minimum for gloved use
- Feature 16 (METRC tag assignment camera flow) may be incomplete â€” no dedicated route in App.jsx, only embedded in ContainerDetail (not fully verified)

### Files Modified/Created
- `docs/audit-frontend-ux.md` (new â€” 352 lines)
...
### Key Decisions
- PHI/REI/EPA-redirect tests require `vi.mock()` on `fetchFarmstockItem` â€” farmstock is not available in test environment
- `setDB(instance)` export needed in `src/db/index.ts` (one-line change) to inject in-memory DB per test file
- Container infrastructure (all 1,180 containers) is seeded by migrations â€” tests reference existing container IDs
- No-FARMSTOCK-URL behavior (skip PHI checks silently) is a known gap flagged in the plan

### Files Modified/Created
- `docs/test-plan.md` (new â€” 673 lines)

### Notes for Next Tasks
- Next task: implement the test infrastructure (helpers/db.ts, helpers/auth.ts, helpers/fixtures.ts)
- Then implement tests in priority order: harvest â†’ pesticide â†’ batches â†’ plant-loss â†’ containers â†’ tag-assignments â†’ foliar â†’ fertigation
...
- Audited exports.ts in detail â€” confirmed cultivation record does NOT expand recipe ingredients
...
- Produced `docs/audit-regulatory-compliance.md` â€” 509 lines, 7 sections
...
- Product names and EPA reg #s are runtime-fetched from farmstock â€” long-term 5-year retention risk if farmstock unavailable

### Files Modified/Created
- `docs/audit-regulatory-compliance.md` (new â€” 509 lines)
...
- HIGH H1+H2: fix MDA report (add time, add lot_number) â€” one-file fix in exports.ts
- HIGH H7: add METRC UID check before harvest event creation in harvest.ts
- CRITICAL C5: add bulk containerâ†’teardown transition when batch auto-closes in harvest.ts
...
- Created `src/tests/helpers/db.ts` â€” `createTestContext()` / `teardownTestContext()` using in-memory SQLite + Knex migration runner + test user seeding (admin/supervisor/grower)
- Created `src/tests/helpers/auth.ts` â€” `getTestToken()` / `authHeader()` JWT helpers using the app's jwt.sign
- Created `src/tests/helpers/fixtures.ts` â€” factory functions: `createTestStrain`, `createTestBatch`, `advanceBatchTo`, `createHarvestBatch`, `createPlantAssignment`, `putContainerActive`, `putContainerEmpty`, `putContainerTeardown`, `putContainerStartup`, `insertStageOverride`
- Created `src/tests/integration/harvest.test.ts` â€” 34 tests covering: harvest batch status gate, harvest event gating on batch status, final harvest side effects (unassignment, containerâ†’teardown, batch auto-close, harvest-batch auto-complete), waste trim (any batch status, required fields), force-close lifecycle, harvest batch uniqueness rules, and sequence numbers
- Created `src/tests/integration/plant-loss.test.ts` â€” 16 tests covering: basic recording, assignment unassignment with loss reason mapping, container state transition to empty, multi-plant container behavior, METRC sync status, validation errors, and mid-batch replacement
- Created `src/tests/integration/applications/pesticide.test.ts` â€” 14 tests covering: Zod required field validation, EPA redirect from farmstock, PHI compliance check with override, REI computation from applied_at + rei_hours, RUP applicator license requirement, and stage block (Rule 19)
- Created `src/tests/integration/batches.test.ts` â€” 21 tests covering: valid lifecycle transitions, invalid/skipped/backward transitions, transition role requirements, field updates, recipe assignment
- Created `src/tests/integration/containers.test.ts` â€” 15 tests covering: teardown from active/empty, teardown event creation, state validation rejections, startup from teardown, ready sign-off (supervisor), supervisor role requirement
- Created `src/tests/integration/applications/foliar.test.ts` â€” 14 tests covering: required field validation, batch status restrictions, EPA redirect (Rule 13), stage block (Rule 14), response fields

### Key Decisions
- `acquireRawConnection` override on Knex client makes migrations run against the pre-created `new Database(':memory:')` instance â€” this is the only way to share the same SQLite in-memory DB between migrations and the app
- In-memory DB context is created fresh per test (`beforeEach`/`afterEach`) to ensure full isolation
- `putContainerActive()` both updates `cv_container_state` AND inserts a `cv_plant_assignments` row â€” the `active` state constraint requires at least one active assignment
...
- Force-close test: the force-close endpoint auto-creates the new in-progress harvest batch â€” the test was updated to verify this behavior rather than trying to create a second one manually
- `cv_fertigation_recipes` and `cv_foliar_recipes` don't have `updated_at` column (migration 006 didn't add it) â€” fixture INSERT SQL was fixed accordingly
...
- The `createTestContext` helper is designed for reuse â€” any new integration test file imports from `../helpers/db.js`
- Farmstock mocking pattern: set `process.env.FARMSTOCK_URL/KEY`, stub `fetch` globally, clean up in afterEach â€” this pattern is established and works
- `npm test` now runs all 175 tests in ~5.6s â€” fast enough for CI
...
### Key Decisions
- Architecture section documents the Option B (separate DBs, cross-app API) integration decision â€” matches docs/sibling-app-resolution.md approved decision
- Auth documented as PIN-based JWT with three roles (grower/supervisor/admin), not "shared SSO" â€” the SSO described in CLAUDE.md does not yet exist
- All 14 migration files listed individually with their table scope â€” useful orientation for schema archaeology
...
### Notes for Next Tasks
- README references docs/ audit files which contain prioritized fix lists â€” good starting point for next work session
...
- Produced `docs/metrc-integration-design.md` â€” 1,246-line Phase 4 integration design document
- Committed and pushed as `docs: METRC API integration design for Phase 4`

### Key Decisions
- METRC uses batch NAME (not a numeric UID) as the identifier for plant batch operations â€” `metrc_plant_batch_uid` stores the name string, not a separate ID
- `metrc_sync_status` is the queue â€” no separate queue table needed; worker polls existing tables
- Identified critical schema gap: `cv_applications_fertigation`, `_foliar`, `_pesticide`, and `cv_container_amendments` are missing `metrc_sync_status` and `metrc_synced_at` â€” requires migration `016_additive_sync.ts` before Phase 4 begins
- Sync ordering invariant: batch creation â†’ phase changes â†’ location moves â†’ harvest batch creation â†’ harvest plant events â†’ additives â†’ waste/loss
...
### Files Modified/Created
- `docs/metrc-integration-design.md` (new â€” 1,246 lines)
...
- `toMetrcPhase()` and `makeBatchName()`/`makeHarvestBatchName()` in `src/lib/domain-utils.ts` are the canonical source for METRC name/phase formatting â€” use them in the sync worker
- The sandbox URL is `https://sandbox-api-mn.metrc.com` â€” sandbox credentials require separate request to MN OCM
- `cv_locations.metrc_name` stores the exact METRC room name â€” must match METRC account configuration exactly; values are currently placeholders (see migration 011)
...
- Offline queue uses IndexedDB (idb library) not localStorage â€” survives page reload
...
- Migration 016 default: `not_required` for existing additive records â€” correct, avoids retroactive METRC backfill
...
### Notes for Next Tasks
- Phase 2 recommended start: Feature 2.1 (Sub-zone Field Maps) is smallest/most self-contained â€” good first Phase 2 feature
- Offline hardening (2.3) is the most complex Phase 2 feature â€” allow 5 days, affects all form entry paths
- Migration 016 content is spelled out in the roadmap â€” copy-paste ready for implementation
...
### Files Modified/Created
- `docs/backlog.md` (new â€” 748 lines)
- `.claude/session_context.md` (this entry)

### Notes for Next Tasks
- Highest-impact quick wins: CRIT-09 (one-liner batch_id fix), CRIT-01 (one-liner auth middleware), DEBT-01 (migration 015 â€” all content in audit-schema-performance.md Section 6)
- Most complex Phase 1 fix: HIGH-08 (PDF cultivation record) â€” needs server-side PDF tooling decision
...
- Produced `docs/ocm-reporting-requirements.md` (1,030 lines, 6 sections):
  - **Section 1:** Regulatory framework â€” 342.25 (what inspectors ask for, retention, production timelines), Rule 4770 (unified crop input log requirements), 18B.37 (full field-by-field table with current status), METRC (what OCM sees directly vs. what they request), MDA (separate enforcement authority, inspection triggers)
  - **Section 2:** Inspection scenario analysis â€” 8 OCM inspection questions (A1â€“A8), 5 MDA inspection questions (B1â€“B5), 4 METRC reconciliation questions (C1â€“C4); each maps to specific report and identifies current capability vs. gap
  - **Section 3:** 11 report specifications â€” each with purpose, requestor, trigger, format, target production time (< 30â€“60s), filter params, required fields with data sources, and gap analysis
  - **Section 4:** OCM Compliance Dashboard design â€” 8 panels with RAG status logic, SQL sketches for each panel query, layout diagram, and the `/api/reports/compliance-status` route spec
  - **Section 5:** Report gap matrix (11 reports Ã— 4 dimensions) + 5 critical schema changes needed (product snapshots, additive sync columns, updated_at, tag assignment sync, user license)
...
### Files Modified/Created
- `docs/ocm-reporting-requirements.md` (new â€” 1,030 lines)
- `.claude/session_context.md` (this entry)

### Notes for Next Tasks
- Implementation priority: Reports 1, 3, 5, 6, 7 are S-effort (new routes, no schema changes) â€” good quick wins
...
- Dashboard (`GET /api/reports/compliance-status`) is a single-query aggregation â€” can be built before individual report routes
...
- **`src/lib/domain-utils.ts`**: Added calcDewPoint() (Magnus formula, Â°C in / Â°F out), calcVPD() (Tetens, 3 decimal kPa), celsiusToFahrenheit() (1 decimal).
...
- **`src/api/routes/sensors.ts`**: 8 routes at /api/sensors â€” GET / (list), POST /sync (upsert from SensorPush), GET /assignments, POST /assignments (assign with auto-unassign), DELETE /assignments/:id, GET /current (Option C on-demand poll with 5-min cache, 30-min stale threshold), GET /:id/readings (raw 7-day limit or hourly), POST /poll (admin trigger).
...
- Both PesticideNew and FertigationNew already had their own `activeBatch` variable â€” sensor hook uses `(lockedBatch || selectedBatch)` inline to avoid naming conflict.
...
- `src/api/app.ts` (import + register)
- `src/tests/unit/domain-utils.test.ts` (new â€” 16 tests)
...
### Notes for Next Tasks
- Background polling (`src/sensor-poller.ts`) is the standalone export â€” wire it to a Task Scheduler script or Railway cron in a separate step (no cron configured yet).
- The Today screen "Current Conditions" panel (Section 5 of design doc) is not yet implemented â€” requires adding `api.getCurrentConditions()` call to `Today.jsx`.
- `sensor_reading_id` FK linkage on application tables (design doc Section 8, audit chain) is not implemented â€” would need migration 017.
- SensorManagement hardcodes the 11 location names (from migration 011 seed) rather than fetching from a `/api/locations` endpoint â€” that endpoint doesn't exist yet.
...
- Produced `docs/sensorpush-integration-design.md` â€” 1,167-line design document covering all 8 sections
...
- Migration numbered `016_sensors.ts` â€” creates `cv_sensors`, `cv_sensor_location_assignments`, `cv_sensor_readings`, and `cv_sensor_readings_hourly` tables
- Readings are denormalized at ingest time: `location_id` and `sub_zone_id` copied from the active assignment onto each reading row â€” makes historical queries fast without joining through assignment history
...
### Files Modified/Created
- `docs/sensorpush-integration-design.md` (new â€” 1,167 lines)
- `.claude/session_context.md` (this entry)

### Notes for Next Tasks
- Phase 2 implementation starts with migration `016_sensors.ts` + `POST /api/sensors/sync`
- `src/lib/sensor-thresholds.ts` is a new file to create â€” VPD optimal ranges per batch status
- Auto-fill hook pattern is documented in Section 4 â€” copy into `PesticideNew.jsx` first (highest compliance value), then `FertigationNew.jsx` and `HarvestDashboard.jsx`
- `SensorBadge` component is a small shared component in `client/src/components/`
- The SensorPush auth flow has a known quirk: access tokens may not need "Bearer" prefix â€” verify during implementation against current SensorPush API docs
- Wind speed is NOT available from SensorPush â€” `wind_speed_mph` on pesticide applications remains manually entered
...
- Added 4 backend endpoints to `src/api/routes/exports.ts` (under existing `/api/exports` prefix):
  - `GET /compliance-dashboard` â€” 8-panel RAG-status aggregation (active REIs, PHI watch, METRC pending/failed, untagged plants, missing batch UIDs, losses unsynced, waste pending disposal)
  - `GET /plant-inventory` â€” all non-closed batches with derived counts (plant_count_current, tagged_count), days in stage, current recipe, REI flag, METRC UID status
  - `GET /tag-verification` â€” active plant assignments joined with container/row/batch/strain; optional sub_zone_id filter; JSON or CSV
  - `GET /metrc-reconciliation` â€” sync status counts + pending/failed item lists for phase_history, location_history, harvest_events, waste_trim, plant_loss
...
- Created 4 frontend pages under `client/src/pages/compliance/`:
  - `ComplianceDashboard.jsx` â€” route `/compliance`; panel grid; auto-refresh every 5 min; overall status banner (green/amber/red)
  - `PlantInventory.jsx` â€” route `/compliance/plant-inventory`; table with stats summary; print button; links to batch detail on row click
  - `TagVerification.jsx` â€” route `/compliance/tag-verification`; sub-zone filter; grouped by sub-zone â†’ row; last 4 of tag shown bold; CSV download
  - `MetrcReconciliation.jsx` â€” route `/compliance/metrc-reconciliation`; summary bar; accordion sections per event type with pending/failed item tables
...
### Notes for Next Tasks
- `/compliance` is accessible to all authenticated users (no minRole guard) â€” consider adding supervisor or admin gate if needed
- CSV download for tag verification uses `window.open()` (same pattern as existing exports) â€” this sends no auth header and will 401 for authenticated routes; a proper blob-fetch approach would be needed to fix this for all export CSV downloads
- `getCount()` helper uses SQLite's `COUNT(*) AS cnt` pattern â€” must use `cnt` alias consistently
...
- Produced `docs/mix-calculator-design.md` (847 lines) with 9 sections:
  - **Section 1:** Problem statement â€” what the calculator solves
  ...
  - **Section 3:** Complete unit conversion tables â€” all rate_unit values to mL/mL canonical form, all volume unit relationships, worked examples for each conversion
  ...
  - **Section 5:** UI design â€” route `/recipes/calculator`, 4 entry points with wiring specifics, layout wireframe, scenario-specific input controls, recipe selector, ingredient list display, UX rules
  - **Section 6:** Print mixing card design â€” Fraunces/JetBrains Mono style matching existing recipe print cards, `window.print()` implementation using `.hidden.print:block` pattern already in FertigationRecipeDetail
  - **Section 7:** plants_per_container awareness â€” 3-tier data source priority (active batch, URL param, manual), 3 rate specifiers (gal/plant, gal/container, gal/gal-pot)
  - **Section 8:** 3 complete worked examples with full math (Z1A auto sub-zone imperial, 3-container metric, 25-gal foliar metric)
  - **Section 9:** Implementation notes â€” frontend-only (no new API routes), extraction module path (`client/src/lib/mix-calculator.js`), exact code for "Use This Volume" sessionStorage handoff

### Key Decisions
- No backend routes needed â€” all data is available from existing endpoints (recipe detail already includes item_name)
- Canonical intermediate is mL/mL ratio â€” all rate_units convert to this form first, then scale and re-express
...
### Files Modified/Created
- `docs/mix-calculator-design.md` (new â€” 847 lines)
- `.claude/session_context.md` (this entry)

### Notes for Next Tasks
- Implementation: create `client/src/lib/mix-calculator.js` + `client/src/pages/recipes/MixCalculator.jsx` + unit tests
- The `RATE_UNIT_LABELS` map in FertigationRecipeDetail is the canonical list of rate_unit values in use â€” matches what is in Section 3 of the spec
- "Use This Volume" sessionStorage keys: `cv_calc_volume_gal` and `cv_calc_volume_batch_id` â€” FertigationNew.jsx must consume these on mount
- Sub-zone container counts are compile-time constants (from seed data) â€” no API call needed for A=150/B=145
...
- Updated `client/src/hooks/useCurrentConditions.jsx`: SensorBadge now accepts `manual` prop â€” shows 'ðŸ“¡ Auto-filled Â· N min ago' (green) or 'âœ Manual entry' (gray) instead of hiding on manual edit.
- Updated `client/src/pages/Today.jsx`: added "Current Conditions" section below Active Batches â€” one card per unique sub_zone_id from active batches. Collapsed on mobile (tap to expand), expanded on tablet.
...
- Updated `client/src/pages/compliance/ComplianceDashboard.jsx`: Environmental Alerts panel added â€” calls getCurrentConditions + getBatches, checks: sensor offline >30min (amber), VPD out of range for current batch stage (red/amber), battery <20% (amber). Placed before Quick Links section.
...
- Updated `client/src/pages/admin/SensorManagement.jsx`: "View History â†’" link to /admin/environmental-history.
- Updated `client/src/App.jsx`: registered /admin/environmental-history route.

### Key Decisions
- VPD_RANGES defined inline in CurrentConditionsCard and ComplianceDashboard (not a shared file) â€” they're simple agronomic constants, duplicating is fine.
- Offline threshold = 30 min (1800s); stale warning = 10 min (600s). Same threshold as design spec.
- SensorBadge `manual` prop: if reading is not null but manual=true, shows 'âœ Manual entry'. If reading is null, returns null (no badge â€” sensor never provided data for this form session). This lets the field badge state be stateful across editing.
- The Current Conditions section on Today uses tab-style collapse via CSS (hidden/md:block) â€” no JS state needed for tablet expansion.
...
- The `battery_pct` field in getCurrentConditions response â€” the endpoint currently doesn't return it (only the sensor list endpoint does). If battery alerts are needed from getCurrentConditions, the endpoint needs a JOIN to cv_sensors. Currently battery alerts silently do nothing if battery_pct is null.
- The Today screen Conditions section uses `hidden md:block` for tablet auto-expansion â€” this is CSS-only. The `conditionsExpanded` state only controls the mobile toggle. The md+ breakpoint always shows it.
- EnvironmentalHistory caps table display at 500 rows but fetches all from API. For large datasets the API may return thousands â€” the 7-day limit on raw readings in the sensor route (`ORDER BY observed_at DESC LIMIT 500` or similar) should be checked if performance is an issue.
...
- Produced `docs/uem-architecture.md` (1,374 lines, 8 sections):
  - **Section 1:** SOP â†’ Skill â†’ Checklist pipeline â€” 4 stages from SOP authoring through skill distribution; how `IntelligenceService.extractSkill()` maps to existing ExtractType infrastructure.
  - **Section 2:** How skills drive sub-applications â€” dynamic form generation via `<SkillForm>`, dynamic checklist generation, validation layer wrapping Zod, graceful degradation.
  - **Section 3:** Agent integration â€” 4 agent types (Skill Extraction, Skill Execution, Compliance Monitoring, Skill Update); human-in-the-loop spectrum table mapping each cultivate workflow to automation level (fully automated â†’ human-only); Felix task format examples.
  - **Section 4:** Full TypeScript skill schema spec â€” all interfaces (SkillSchema, SkillStep, FieldDef, FieldValidation, AutoFillSource, OutputAction, PostCondition, ComplianceCheck); Expression DSL with context variables and supported operations; severity level table.
  - **Section 5:** ff-dcs â†” cultivate integration points â€” new ff-dcs API routes, two new ff-dcs database tables (skills, skill_runs), cross-app service auth pattern, webhook notification design, compliance Q&A integration.
  - **Section 6:** Regulatory alignment â€” comparison to standard operations, per-regulation alignment table (342.25, 4770, 18B.37, METRC, MDA), deviation documentation structure.
...
- Committed and pushed as `docs: Uniform Enterprise Management architecture design â€” SOP-driven skill system`.

### Key Decisions
- ff-dcs is the authoritative home for skill schemas (skills table in ff-dcs DB), not cultivate or a separate service. The regulatory argument (SOP â†’ skill â†’ record) only holds when the skill lives next to the SOP.
- `IntelligenceService.extractSkill()` would be a new method that calls existing `extract()` with multiple types + a second LLM pass for synthesis â€” extending existing infrastructure, not replacing it.
- Expression DSL is a simple string format (not JSON Logic) â€” designed to be parseable by a small evaluator without a library dependency.
...
### Files Modified/Created
- `docs/uem-architecture.md` (new â€” 1,374 lines)
- `.claude/session_context.md` (this entry)
...
- Skills API in cultivate: startup fetch â†’ localStorage cache â†’ ETag refresh. Pattern is in Appendix B.
- The 8 open questions in Section 8 are decision gates for operator before Phase 2 code work begins

---

## Task: OpenAPI spec â€” skeleton + auth/batches/strains
**Completed:** 2026-05-21

### What Was Done
- Created `docs/openapi.yaml` â€” OpenAPI 3.0.3 skeleton with full documentation for 13 routes across 3 files
- **Auth (3 routes):** `GET /auth/users` (no auth â€” documents the CRIT-01 security finding), `POST /auth/login` (PIN auth, lockout logic, 423 response), `POST /auth/refresh`
...
- Trailing comment: `# TO BE CONTINUED â€” remaining routes in subsequent tasks`
...
### Files Modified/Created
- `docs/openapi.yaml` (new â€” 912 lines)
- `.claude/session_context.md` (this entry)
...
- The `Batch` schema covers all fields returned by the enriched SELECT â€” no fields missing
...
- Produced `docs/agent-sdk-design.md` (1,869 lines, 8 sections):
  - **Section 1:** Cultivate MCP Server design â€” 22 tools (11 read, 8 write, 3 utility) with full input schemas, descriptions, approval levels, and handler patterns. Key design: handlers call shared query functions (not HTTP routes) to avoid round-trips.
  - **Section 2:** Approval gate architecture â€” APPROVAL_REQUIRED map, cv_agent_approval_queue table schema, buildApprovalGate() PreToolUse hook that pauses agent execution pending supervisor review, PATCH /api/agents/approvals/:id resolve endpoint, WebSocket notification flow
  - **Section 3:** Audit trail design â€” cv_agent_audit_log table, buildAuditHook() PostToolUse hook, provenance model (human-initiated vs agent-initiated), MN 342.25 compliance argument for agent records
  - **Section 4:** Skill execution agent pattern â€” SkillContext/SkillExecutionResult types, full executeSkill() implementation, buildSkillSystemPrompt() construction, skill JSON file storage
  - **Section 5:** Event trigger architecture â€” cv_agent_triggers table with condition_json per trigger type, runEventDispatcher() service, evaluateTriggerCondition() per sensor_threshold/state_change/scheduled, trigger examples table
  - **Section 6:** 4-phase implementation plan (Foundation â†’ Write+Approvals â†’ Skill Execution â†’ Event Triggers) with concrete file lists, commit targets, and proof-of-concept tests
  - **Section 7:** Felix vs Agent SDK division of labor table with heuristics and hybrid fertigation pattern
  - **Section 8:** Security and permissions â€” user identity flow, role enforcement in MCP tools, approval gate role check, audit provenance table, rate limiting and safety limits (50 tool calls/session, 2 concurrent sessions/user)
...
### Key Decisions
- MCP tool handlers call shared query functions directly in-process (not over HTTP) â€” avoids round-trips and ensures route/tool logic stays in sync
- Approval gate uses DB polling (3s interval) as primary mechanism + WebSocket as wake signal â€” resilient if WebSocket drops
- The system_agent service account holds role 'grower' by design â€” forces supervisor approval for any compliance-critical write even from automated triggers
- Shared query functions (`src/api/queries/`) need to be extracted from inline route handlers as Phase 1 implementation work â€” this is the key refactoring prerequisite

### Files Modified/Created
- `docs/agent-sdk-design.md` (new â€” 1,869 lines)
- `.claude/session_context.md` (this entry)
...
- The 22 tool definitions in Section 1 are the complete spec â€” implement them in order: read tools first, utility tools, then write tools
- `POST /api/agents/query` endpoint (the Phase 1 PoC) should be the first testable artifact
- Skills JSON files go in `skills/` at project root (not `src/skills/`) â€” referenced by skill-loader.ts
...
### What Was Done
- `client/src/lib/mix-calculator.js` â€” Pure calculation module: `CONVERSIONS`, `SUB_ZONE_CONFIG`, `rateToMlPerMl` (16 rate_unit variants), `isWeightBased`, `formatIngredientQty` (imperial/metric auto-unit selection with boundary thresholds), `formatVolume`, `calcMix`, `calcTargetVolumeMl`
- `client/src/tests/mix-calculator.test.js` â€” 96 unit tests covering all functions, imperial/metric auto-unit thresholds, weight-based ingredients, and all three worked examples from design doc Â§8
- `client/src/components/MixCalculator.jsx` â€” Self-contained calculator component with 4 scenario modes (sub-zone, rows, plant count, manual), live memoized calculation, `RateInputs` subcomponent with per-plant/per-container rate units, localStorage draft persistence, print-to-new-window mixing card (Fraunces/JetBrains Mono/earthy palette), 'Use This Volume' callback for return flow
- `client/src/pages/recipes/MixCalculatorPage.jsx` â€” Standalone page at `/recipes/calculator`; recipe type toggle (fertigation/foliar), recipe selector from API, loads recipe+ingredients and embeds MixCalculator; handles `return_to` param via sessionStorage + navigate
- `FertigationRecipeDetail.jsx` â€” 'Mix Calculator' button added alongside Print Recipe Card
- `FoliarRecipeDetail.jsx` â€” Same
- `FertigationNew.jsx` â€” `Link` import added; sessionStorage consumer on mount reads `cv_calc_volume_gal`; 'Calculate mix â†’' link in recipe display chip
- `RecipeIndex.jsx` â€” Mix Calculator entry card with FlaskConical icon
- `App.jsx` â€” `/recipes/calculator` registered before dynamic recipe `:id` routes

### Key Decisions
- No new backend routes â€” all data from existing `getFertigationRecipe`/`getFoliarRecipe` endpoints which already include `item_name`
- Canonical intermediate is mL/mL ratio â€” all rate_units convert to this form; weight-based units (g_per_gal, g_per_L) use 1gâ‰ˆ1mL assumption and output in g/kg
...
- Print card opens new window with Google Fonts + `window.print()` â€” same result as hidden print:block but simpler for a sub-page
- sessionStorage handoff (not URL params) for "Use This Volume" â€” keeps URL clean

### Files Modified/Created
- `client/src/lib/mix-calculator.js` (new)
- `client/src/tests/mix-calculator.test.js` (new â€” 96 tests)
...
- FoliarNew.jsx does not yet have a 'Calculate mix â†’' link â€” could be added following the same pattern as FertigationNew if the foliar application form has an active_recipe_id equivalent
...
- **Bug 2 (final_harvest batch status gate):** Same route â€” `final_harvest` is now blocked unless `batch.status === 'harvesting'`.
- **Bug 3 (manicure terminology):** Replaced all user-visible "Manicure Batch (MB)" strings with "Partial Harvest Batch (MB)" across: `PartialHarvestForm.jsx` (1), `HarvestDashboard.jsx` (4 â€” toast, button text, batch type display, title tooltip), `WeatherEventClose.jsx` (1). METRC `batch_type = 'manicure'` remains as the internal data value.
...
- `src/api/routes/sensors.ts` has uncommitted changes from a prior session (Zod validation improvements) â€” commit those separately
- The harvest.test.ts integration tests still pass because the test fixtures advance batches to 'harvesting' before creating harvest events â€” both checks are satisfied by the existing fixture setup

---

## Task: Skill schema proof of concept â€” pesticide application
**Completed:** 2026-05-21

### What Was Done
- Created `src/skills/pesticide-application.skill.json` â€” hand-crafted skill schema with 5 preconditions (check_id dispatch keys), 6 steps, sensor auto-fill directives, 2 outputs, 1 post-condition, 2 compliance checks
- Created `src/lib/skill-loader.ts` â€” loads `*.skill.json` files from `src/skills/` at startup; `getSkill()`/`listSkills()` functions; designed for Phase 2 upgrade to ff-dcs API fetch
- Created `src/lib/skill-validator.ts` â€” evaluates preconditions against live DB data; dispatches on `check_id`; returns `ValidationResult` with per-check pass/fail/message/severity
- Created `src/api/routes/skills.ts` â€” `GET /api/skills` (list), `GET /api/skills/:id` (detail), `GET /api/skills/:skillId/validate?batch_id=X&input_id=Y` (real-time validation + sensor auto-fill)
- Created `src/api/routes/skill-instances.ts` â€” `GET /api/skill-instances` (SOP compliance evidence query)
- Created `src/db/migrations/017_skill_instances.ts` â€” `cv_skill_instances` table
- Updated `src/api/routes/pesticide-applications.ts` â€” creates `cv_skill_instances` record on every successful POST (best-effort, wrapped in try/catch)
- Updated `client/src/pages/applications/PesticideNew.jsx` â€” added `SkillValidationPanel` component; calls `validateSkill` on batch/product selection; displays live precondition badges; blocks Save if `skillBlocked`
- Updated `client/src/api.js` â€” 4 new skill methods
- Updated `docs/uem-architecture.md` â€” added Appendix C documenting the POC, how validation works, skill instance evidence trail, and ff-dcs integration delta for Phase 2
...
- Validation dispatches on `check_id` string field added to preconditions (not in original Appendix A) â€” enables extensible evaluator without a full DSL parser
- Skill instance creation in POST handler is best-effort (try/catch, `app.log.warn` on failure) â€” never fails the compliance application record
...
- `getBatchStageKey()` is duplicated in `skill-validator.ts` (also in `pesticide-applications.ts`) â€” flagged as tech debt to extract to `domain-utils.ts`
...
- `getBatchStageKey()` is duplicated in validator and pesticide route â€” extract to `domain-utils.ts` when time allows
...
- The POC validates: (1) skill JSON loads correctly, (2) preconditions evaluate against live data, (3) frontend shows real-time badges, (4) evidence trail is created â€” all 4 architecture claims are demonstrated

---

## Task: OpenAPI spec â€” containers + applications
**Completed:** 2026-05-21
...
- Documented 28 routes total across: containers (7), fertigation-applications (4), foliar-applications (4), pesticide-applications (5), container-amendments (4 â€” wait, 8 total counting amendments)
...
### Key Decisions
- Used `{id}` path parameter for container routes (type: string) â€” documented as Z{zone}-{sub}-R{row}-C{position} format
...
### Files Modified
- `docs/openapi.yaml` â€” 1,560 lines added (now 2,472 lines); committed and pushed
...
- The `{id}` naming is consistent for all paths â€” keep that convention
...
- Produced `docs/sso-design.md` â€” 619-line design document covering 8 sections

### Key Decisions
- Phase 1 (hours): shared JWT_SECRET + `.hatstak.app` domain httpOnly cookie. Cultivate â†” farmstock SSO with no new infrastructure.
...
- Unified role set: grower/supervisor/admin (cultivate names win; farmstock renames workerâ†’grower, managerâ†’supervisor).
- CRIT-01 (`GET /api/auth/users` unauthenticated) must be fixed before cookie auth is added â€” cookie SSO would otherwise make the user list accessible cross-app without login.

### Files Modified/Created
- `docs/sso-design.md` (new â€” 619 lines)
- `.claude/session_context.md` (this entry)
...
- CRIT-01 fix (one-liner): add `{ preHandler: requireAuth }` to `app.get('/users', ...)` in cultivate's auth.ts â€” must ship before Phase 1
- Cookie name: `hatstak_token` (shared across all subdomains)
- localStorage keys remain: `cv_token` (cultivate), `fs_token` (farmstock) â€” offline-resilient fallback

---

## Task: OpenAPI spec â€” observations, recipes, harvest, exports, sensors (complete)
**Completed:** 2026-05-21
...
- Appended 3,138 lines to docs/openapi.yaml:
  ...
  - **Routes for all 9 route files** â€” 56 new routes total
- Removed `# TO BE CONTINUED` sentinel â€” spec is now complete
- Committed and pushed as `docs: OpenAPI spec â€” observations, recipes, harvest, exports, sensors (complete)`

### Key Decisions
- Fertigation recipes are at `/recipes/fertigation` (not `/fertigation-recipes`) â€” matches actual app.ts prefix `/api/recipes/fertigation`
- Foliar recipes at `/recipes/foliar` (not `/foliar-recipes`) â€” matches `/api/recipes/foliar`
- Observations at `/observations`, harvest at `/harvest`, plant-loss at `/plant-loss`, etc. â€” all correct from app.ts
- Previous task documented applications at `/fertigation-applications` etc. â€” those paths don't match app.ts (`/api/applications/fertigation`) but were not corrected in this task
- HarvestBatch.batch_type uses `harvest`/`manicure` as stored in DB â€” documented that UI should use "partial harvest" not "manicure"

### Files Modified
- `docs/openapi.yaml` â€” 3,138 lines added; spec is now complete (5,609 lines total)

### Notes for Next Tasks
- The application routes in the existing spec use wrong paths (`/fertigation-applications` etc.) instead of correct `/applications/fertigation` â€” a future correction task should fix these
- container-lifecycle routes (teardown, soil sample, startup, startup-ready) are NOT yet documented â€” these are registered at `/api/containers` alongside containers.ts
...
- Created `client/src/pages/planting-plans/PlantingPlanList.jsx` â€” route `/planting-plans` (supervisor). Status filter tabs (draft/active/superseded/all). Cards show strain, sub-zone, version, committed/draft counts.
- Created `client/src/pages/planting-plans/PlantingPlanNew.jsx` â€” route `/planting-plans/new?batch_id=X` (supervisor). Loads batch from query param + container summary for ready-count badges on each sub-zone. Draft persistence via `cv_draft_planting_plan_new`.
- Created `client/src/pages/planting-plans/PlantingPlanDetail.jsx` â€” route `/planting-plans/:id` (supervisor). Container grid grouped by row, color-coded (green=ready/addable, blue=draft, amber=committed, gray=n/a). Click green to add, click blue to toggle selection. Draft items list with checkboxes. "Commit Selected (N)" and "Commit All (N)" fixed bottom action bar. "New Version" and "Cancel Plan" secondary buttons.
- Updated `client/src/pages/batches/BatchDetail.jsx`: added planting plan section for germ/seedling/cult-hoop/field-veg statuses. Loads most recent plan via api.getPlantingPlans({ batch_id }). Shows status badge, committed/draft counts, and "Open Plan Builder â†’" link (supervisor). Shows "Create" link when no plan exists.
- Updated `client/src/App.jsx`: 3 imports + 3 routes (all minRole="supervisor").

### Key Decisions
- All planting-plan routes require `minRole="supervisor"` â€” matches backend `requireRole('supervisor')` on all write endpoints
- `addPlantingPlanItem` wraps single item in `{ items: [item] }` format per the bulk endpoint contract
- Container grid uses horizontal overflow-x-scroll per row (30 containers/row Ã— 44px = too wide for mobile, scrollable on tablet)
...
- `api.getContainerSummary()` call in PlantingPlanNew uses the `/containers/summary` endpoint which requires no sub_zone_id param â€” returns all 8 sub-zones with state counts
- Plan builder scrolls horizontally per row â€” works well on tablet (supervisor device). Phone users would need to scroll each row separately.
- After a commit, the batch transitions from cult-hoop â†’ field-veg automatically (handled by the commit endpoint, not the UI)
...
## Task: Pest identification agent â€” discovery and design
**Completed:** 2026-05-21
...
- Produced `docs/pest-identification-agent-design.md` (894 lines, 10 sections + appendix):
  - **Section 1:** Purpose and scope â€” the compliance problem (misclassification path to regulatory failure), what the agent does vs. does not do
  - **Section 2:** Three integration points â€” Path A (post-observation trigger, MVP), Path B (in-observation entry, Phase 2), Path C (standalone scan mode, Phase 2); full workflow state machine
  - **Section 3:** MVP design â€” text-only, `IdentificationResult` TypeScript interface, system prompt architecture, catalog matching via keyword approach, compliance pre-checks per matched product; MVP feature table
  - **Section 4:** Phase 2 vision capability â€” why vision matters (mites vs mites, mildew vs deposits), Claude multi-modal invocation pattern, 4-photo guided capture protocol, confidence improvement rationale
  - **Section 5:** Data model â€” new `cv_pest_id_sessions` table schema + rationale for separate table vs cv_skill_instances; migration number 018; no changes to existing tables
  ...
  - **Section 7:** UEM skill schema fit â€” position as Skill Execution Agent feeding PesticideApplication skill; draft skill schema JSON; human-in-the-loop spectrum; three-record compliance evidence chain (observation â†’ session â†’ skill_instance â†’ application)
  - **Section 8:** Field UX design â€” post-observation trigger layout, PestIdFlow screen (3 sections), results screen mockup (IPM first then pesticide), pre-fill flow into PesticideNew.jsx
  ...
  - **Section 10:** Implementation phasing â€” Phase 1 MVP (M effort: 8 files, 1-2 days), Phase 2 vision (M-L), Phase 3 UEM skill schema integration (S)
  - **Appendix:** Cannabis cultivation pest reference table (7 pests, 5 diseases, 5 deficiencies) intended as system prompt seed

### Key Decisions
- claude-sonnet-4-6 selected over specialized plant disease APIs â€” Claude has broader cannabis knowledge and can factor in compliance context (batch stage, PHI, farmstock catalog)
- New `cv_pest_id_sessions` table (not reusing cv_skill_instances) â€” pest ID sessions are exploratory/advisory with variable outcomes; skill_instances record SOP compliance execution
- Prompt caching applied to system prompt (stable cannabis pest knowledge base) â€” significant cost reduction for consultation-frequency calls
...
- Compliance evidence chain: observation â†’ pest_id_session â†’ skill_instance â†’ pesticide_application creates a machine-readable audit trail satisfying MN 18B.37 "why this product" question

### Files Created
- `docs/pest-identification-agent-design.md` (new â€” 894 lines)
- `.claude/session_context.md` (appended this entry)

### Notes for Next Tasks
- Phase 1 implementation: 8 files needed â€” migration 018, system prompt, pest-identifier.ts, pest-id routes, api.js methods, PestIdFlow.jsx, observation trigger, PesticideNew.jsx pre-fill params
- Operator must decide Q1 (IPM log record) and Q3 (confidence gating) before Phase 1 build begins
- System prompt seed is in Appendix â€” 17 cannabis pests/diseases/deficiencies ready to expand into full prompt
- `src/agents/prompts/pest-identification.md` is the home for the versioned system prompt
- The `triggered_app_id` field on cv_observations is already schema-ready for linking observation â†’ application; the agent's job is to populate it
...
- Produced `docs/ux-field-analysis.md` (842 lines, 11 sections):
  ...
  - Three-tap audit for 8 daily workflows â€” current vs. achievable tap counts
...
### Key Decisions
- FinalHarvestForm and PartialHarvestForm rated C â€” compliance-critical forms must survive network drops; missing draft + offline detection is a real-world risk
- "Pending sync" cosmetic indicator is misleading â€” records are NOT actually queued for retry; only drafts preserved. Flagged before real-world use.

### Files Modified/Created
- `docs/ux-field-analysis.md` (new â€” 842 lines)

### Notes for Next Tasks
- Highest-priority next action: implement the 10 quick wins (Section 11) â€” ~90 min, clears 9 P0 items
- P0-04/P0-05 (REI pre-entry check) require reading ContainerDetail.jsx:150+ and ContainerScanner.jsx for mount point
- P0-08 "Manicure â†’ Partial Harvest Batch" is a UI label change only â€” DB value stays 'manicure', no migration needed
...
## Task: Pest identification agent â€” design review
**Completed:** 2026-05-21
...
- Reviewed the document (894 lines, 10 sections + appendix) against requirements:
  - âœ… Identification capability: claude-sonnet-4-6 text+vision, tool-use structured output, `IdentificationResult` interface
  - âœ… Integration points: three paths (post-observation MVP, in-form Phase 2, standalone Phase 3)
  - âœ… Output: structured ID with confidence, IPM-first path, pesticide pre-fill into PesticideNew.jsx
  - âœ… Compliance: target_pest populates MN 18B.37 field; three-record chain (observationâ†’sessionâ†’skill_instanceâ†’application)
  - âœ… Data model: new `cv_pest_id_sessions` table, migration 018, no changes to existing tables
  - âœ… Phasing: MVP (text, M effort), Phase 2 (vision, M-L), Phase 3 (UEM skill schema, S)
  - âœ… UEM fit: Skill Execution Agent precursor to PesticideApplication skill; human-in-the-loop at "Assisted" level
  - âœ… Open questions: 6 operator decisions identified (Q1: IPM log record; Q3: confidence gating are prerequisites for build)

### Key Decisions (pre-existing, confirmed)
- claude-sonnet-4-6 with prompt caching on system prompt (stable pest knowledge base)
- Separate `cv_pest_id_sessions` table (not reusing cv_skill_instances) â€” sessions are exploratory/advisory
- IPM-first design with deliberate friction before the pesticide path
- Path A (post-observation trigger) is MVP â€” no observation form changes required
...
- System prompt seed is in Appendix A of the design doc â€” 17 cannabis pests/diseases/deficiencies ready to expand
- Prompt version should be stored in cv_pest_id_sessions.context_snapshot for audit reproducibility (Q6 answer: yes)

---

## Task: Pest identification agent â€” discovery and design (re-run verification)
**Completed:** 2026-05-22
...
- No new document was produced â€” the prior task completed this work in full
- Session context already contained the prior design and review entries

### Key Decisions
- No new decisions â€” existing document stands
...
- Migration number is 018 (`018_pest_id_sessions.ts`) â€” confirm no migration was added between 017 and 018 before implementing

---

## Task: Pest identification agent â€” design confirmation (third pass)
**Completed:** 2026-05-22
...
### What Was Done
- Prior session (c5b7724) had already applied all three bug fixes: batch status gates in `harvest.ts` and manicureâ†’"Partial Harvest Batch" display strings in `HarvestDashboard.jsx`, `PartialHarvestForm.jsx`, `WeatherEventClose.jsx`
- Added 6 integration tests to `src/tests/integration/harvest.test.ts` in a new `'Harvest event â€” batch status gate'` describe block:
...
- Tests for partial_harvest on non-harvesting statuses use `batch_type: 'manicure'` harvest batches â€” consistent with the domain model
...
## Task: Pest identification agent â€” design verification (fourth pass)
**Completed:** 2026-05-22
...
- Read the full `docs/pest-identification-agent-design.md` (894 lines, committed at 3176b91) and confirmed complete coverage of all task requirements:
  - âœ… Identification capability: claude-sonnet-4-6, tool-use structured output, `IdentificationResult` interface (Â§3)
  - âœ… Integration points: 3 paths â€” Path A post-observation (MVP), Path B in-form (Phase 2), Path C standalone (Phase 2) (Â§2)
  - âœ… Output: IPM-first results screen, pesticide pre-fill into PesticideNew.jsx, catalog matching (Â§8)
  - âœ… Compliance: target_pest populates MN 18B.37 field; 4-record evidence chain observationâ†’sessionâ†’skill_instanceâ†’application (Â§7.4)
  - âœ… Data model: `cv_pest_id_sessions` table, migration 018, no changes to existing tables (Â§5)
  - âœ… Phasing: MVP text-only (Phase 1, M effort), vision (Phase 2, M-L), UEM skill schema (Phase 3, S) (Â§10)
  - âœ… UEM fit: Skill Execution Agent precursor to PesticideApplication skill; human-in-the-loop table (Â§7)
  - âœ… Open questions: 6 operator decisions; Q1 (IPM log) and Q3 (confidence gating) gate Phase 1 build
...
- Phase 1 build requires operator Q1 and Q3 decisions (see Â§9 of design doc)
- Migration 018 must not conflict with conflict_log migration planned in roadmap â€” verify latest migration number before writing
...
- Created `client/src/pages/containers/TagAssignmentWalkthrough.jsx` at route `/tag-assignments`:
  ...
  - Fetches `GET /api/tag-assignments/untagged` â†’ rows grouped by `row_id`
...
### Key Decisions
- `assignTagRaw` returns raw `{status, ok, data}` instead of throwing â€” this is intentional to give callers the 409 body (`existing_assignment`) without exception handling gymnastics. Used only for tag assignment, not a pattern change for the rest of api.js.
- `PlacementRow` is a stateful component managing its own tag input, status, and conflict data â€” this prevents parent re-renders from resetting input state while still allowing the parent to track overall count via the `onAssigned` callback.
- `inputRefs` is a `useRef({})` map keyed by `assignment_id` â€” allows auto-advance without traversing the DOM.
...
- The walkthrough supports `?sub_zone_id=Y` filter via the backend (`getUntaggedAssignments` passes all params through) but there's no UI for sub-zone filtering yet â€” could be added as a second select
- Bulk assign (`bulkAssignTags`) is wired in api.js but has no UI yet â€” walkthrough uses one-at-a-time via `assignTagRaw`
- BatchDetail page could benefit from a "Tag Plants â†’" button linking to `/tag-assignments?batch_id=X` when untagged placements exist â€” not built yet
...
### What Was Done
- Created `client/src/pages/locations/LocationView.jsx` â€” read-only location overview at `/locations`
...
- Field section matches `batch.current_location_name` to sub-zone IDs (Z1Aâ€“Z4B) â€” field location names equal sub_zone_id
...
- `client/src/api.js` â€” added getPlantingPlans, getPlantingPlan
- `client/src/App.jsx` â€” import + route `/locations`
- `client/src/components/NavBar.jsx` â€” MapPin import + Locations nav link

### Notes for Next Tasks
- `/planting-plans/:id` and `/planting-plans/new` routes are not registered in App.jsx and no page components exist â€” the Planting Plans UI is still pending
...
- **CRIT-02**: Already committed prior to this session â€” DELETE endpoints removed from `cv_applications_fertigation`, `cv_applications_foliar`, `cv_applications_pesticide`.
...
- **CRIT-05** (`30e71c8`): `GET /api/exports/metrc-waste` added to exports.ts â€” aggregates `cv_plant_waste_trim_events` and `cv_plant_loss_events` in METRC waste format. JSON + CSV. `api.getMetrcWasteExport()` and `api.downloadMetrcWasteCsv()` added.
- **CRIT-06**: Already committed prior to this session â€” harvest.ts Rule 34 cascade transitions all EMPTY containers to TEARDOWN when batch auto-closes.
...
- **CRIT-09** (`21030fb`): BatchDetail amendment quick-action link now passes `?batch_id=` â€” fixes missing `plant_batch_id` on container amendments logged from batch context.
...
- CRIT-03: METRC additives export flattens to ingredient-level rows; cultivation record keeps nested `ingredients` array per application â€” different shapes for different consumer needs.
- CRIT-04: Snapshot is best-effort (null if farmstock unavailable) â€” never fails the application save.
...
### Files Modified/Created
- `src/api/routes/auth.ts` â€” public login-users route + requireAuth on /users
- `src/api/routes/exports.ts` â€” fertigation ingredient expansion + metrc-waste route
- `src/api/routes/pesticide-applications.ts` â€” product snapshot capture
- `src/api/routes/foliar-applications.ts` â€” product snapshot capture
- `src/api/routes/container-amendments.ts` â€” product snapshot capture
- `src/db/migrations/018_product_name_snapshots.ts` â€” new migration
- `client/src/api.js` â€” getUsers â†’ /login-users; getUsersAdmin â†’ /users; new export/waste methods
- `client/src/pages/Today.jsx` â€” error state + retry
- `client/src/pages/batches/BatchDetail.jsx` â€” amendment batch_id fix
- `client/src/pages/applications/PesticideNew.jsx` â€” offline handling
- `client/src/pages/containers/ContainerDetail.jsx` â€” REI pre-entry gate
- `client/src/pages/containers/ContainerScanner.jsx` â€” REI pre-navigation check
...
- `sensors.ts` has uncommitted changes (pre-existing) â€” unrelated to this task

---

## Task: Regulatory compliance test coverage â€” Tier 1 gaps
**Completed:** 2026-05-22
...
- Removed DELETE endpoints from `fertigation-applications.ts`, `foliar-applications.ts`, `pesticide-applications.ts` (Business Rule 5 â€” 5-year retention, no deletion of audit records)
- Added test for `cult-hoop` status blocking `partial_harvest` to `harvest.test.ts`
- Created `src/tests/integration/regulatory-gaps.test.ts` â€” 24 new tests covering:
  - (4) Batch close cascades empty containers to TEARDOWN (3 tests)
  - (5) plants_per_container=2 â€” multi-plant containers (3 tests)
  - (6) DELETE on compliance endpoints returns 404 (3 tests)
  - (7) Waste trim lifecycle: collected â†’ disposed (5 tests)
  - (8) plant_count_current derived from active assignments (3 tests)
  - (9) METRC tag format validation â€” reject non-24-char, special chars; accept valid (6 tests)

### Key Decisions
- DELETE test uses `expect([404, 405]).toContain(res.statusCode)` â€” Fastify returns 404 when no handler is registered; 405 is also acceptable
- Rule 34 cascade only fires when `remainingActive === 0` (batch auto-close) â€” individual final harvests do NOT cascade other containers
- Waste trim 'held' and 'reported' states have no dedicated transition endpoints in Phase 1; tests cover what's implemented (collected â†’ disposed)

### Files Modified/Created
- `src/api/routes/harvest.ts` â€” Rule 34 cascade logic added
- `src/api/routes/fertigation-applications.ts` â€” DELETE endpoint removed
- `src/api/routes/foliar-applications.ts` â€” DELETE endpoint removed
- `src/api/routes/pesticide-applications.ts` â€” DELETE endpoint removed
- `src/tests/integration/harvest.test.ts` â€” cult-hoop test added
- `src/tests/integration/regulatory-gaps.test.ts` (new â€” 24 tests)
...
- The `sensors.ts` file has pre-existing uncommitted changes (from a pre-existing sensor task) â€” unrelated to this task

---

## Task: SSO Phase 1 â€” shared JWT cookie for hatstak.app cross-subdomain auth
**Completed:** 2026-05-22

### What Was Done
- Installed `@fastify/cookie@^11.0.2` as a runtime dependency
- Registered `fastifyCookie` plugin in `src/api/app.ts` (no cookie signing secret â€” JWT is already signed)
...
- Updated `src/api/routes/auth.ts`:
  - Login and refresh both call `setHatStackCookie()` â€” sets `hatstak_token` httpOnly cookie with `domain=.hatstak.app` in prod, `sameSite=lax`, `maxAge=7d`; `domain: undefined` in dev (localhost)
  - Refresh now re-validates user is still active in DB and returns `{ token, worker }` (previously only `{ token }`)
  - Added `POST /logout` endpoint that clears the cookie (no auth required â€” safe)
  - Added `Cache-Control: no-store, private` header on all three auth response handlers
- Updated `client/src/App.jsx`: always calls `api.refreshToken()` on init (no longer gated on localStorage presence) â€” cookie from farmstock/ff-dcs auto-logs user in; `logout()` now calls `api.logout()` fire-and-forget to clear cookie
...
- `refreshToken` now returns `{ token, worker }` â€” backward-compatible since existing callers only destructure `{ token }`
- Logout has no auth requirement â€” clearing a cookie for a non-existent session is a safe no-op
- localStorage remains primary offline credential; cookie is the SSO bridge for cross-subdomain sessions
- CRIT-01 (`GET /api/auth/users` unprotected) was intentionally deferred â€” adding requireAuth would break the login picker since users must load before auth
...
### What Was Done
- **NavBar**: `py-2` â†’ `py-3` + `minHeight: '60px'` â€” meets 56pt gloved-use minimum (CLAUDE.md Rule 1)
- **Batches.jsx**: Header "+ New Plant Batch" button 44px â†’ 56px
- **Today.jsx**: `conditionsExpanded` default changed `false â†’ true` (Current Conditions expanded on mobile by default); "View all â†’" button gets `minHeight: '44px'` tap target; simplified toggle class
- **HarvestDashboard**: All "Manicure Batch (MB)" â†’ "Partial Harvest Batch (PHB)" â€” CLAUDE.md prohibits "manicure" in UI
- **FinalHarvestForm**: Weight unit buttons 32px â†’ 48px; draft persistence (`cv_draft_final_harvest_{batchId}_{assignmentId}`); offline detection ("Do NOT retry" warning); removed `autoFocus` on weight input (triggered keyboard before product chips were visible)
- **PartialHarvestForm**: Weight unit buttons 32px â†’ 48px; draft persistence (`cv_draft_partial_harvest_{batchId}_{assignmentId}`); offline detection
- **PesticideNew**: Category/pest/wind-direction chips 36px â†’ 44px (offline detection was already present from a prior task)
- **FoliarNew**: Purpose chips + category picker chips 36px â†’ 44px
- **BatchNew**: Added `Toast` component + success toast "Batch created âœ“" before navigate; draft persistence (`cv_draft_batch_new`) with 3s debounce + restore on mount
- **BatchDetail**: Added `Toast` component + success toast on phase transitions ("Moved to Field â€” Veg âœ“")
- **PlantLossForm**: Added offline detection â€” network error shows "Network lost â€” draft preserved. Retry when online." without clearing saving state
...
- Tag verification state (`tagConfirmed`) is intentionally NOT persisted in draft â€” operator must re-verify the physical tag each session for safety
...
- "Manicure" terminology: the API still uses `batch_type: 'manicure'` internally â€” only the UI label was changed. If the API is ever updated, HarvestDashboard's `activeMB` find() condition would need updating too.
...
### What Was Done
- Created `src/lib/farmstock-client.ts` â€” shared `triggerFarmstockDepletion(params, logger)` function; fire-and-forget POST to `{FARMSTOCK_URL}/api/stock/deplete`; returns early if no FARMSTOCK_URL/KEY; catches all errors and logs warnings; never throws
...
- Fertigation: no `input_lot_id` field in schema (recipe-level tracking only) â€” no call added

### Key Decisions
- Shared module (`farmstock-client.ts`) rather than duplicating the function 3Ã—; keeps depletion behavior consistent
...
- `FARMSTOCK_URL` was already in `.env.example` â€” no change needed
...
- The `/api/stock/deplete` endpoint must exist in farmstock for this to do anything â€” if farmstock doesn't have that route yet, the depletion will log non-ok status warnings silently
- The depletion body matches the spec: `{ lot_id, quantity, quantity_unit, depleted_by_app: 'cultivate', reference_id, reference_type }`
- Fertigation depletion would require adding `input_lot_id` to `cv_applications_fertigation` schema â€” a future migration if per-lot fertigation tracking is needed
...
- **docs/deployment.md**: Runbook covering prerequisites (Node â‰¥18, Railway CLI), required env vars table, initial dev setup, Railway first-time and ongoing deploys, running the backup (cron examples for both Linux and Windows), Felix health checks, and a troubleshooting section (Felix hung, DB locked, migration failed, Railway health check failures).
- **sensors.ts**: Committed pending Zod validation cleanup for `/current` query params (leftover from prior session).

### Key Decisions
- Bash script uses `sqlite3 .backup` rather than `cp` when sqlite3 is available â€” the `.backup` command is safe with WAL mode and produces a consistent snapshot even under concurrent writes.
- PowerShell script uses `Copy-Item` (no sqlite3 CLI assumed on Windows) â€” safe in dev where the DB is idle during backup.
...
- `.env.example` (updated â€” SENSORPUSH vars added, comments added)
...
- Backup script does not handle the Railway cron scheduling problem â€” Railway's native Jobs feature or a sidecar cron container is needed for production scheduled backups. Documented in docs/deployment.md.
- No `TIMETRACK_URL` or Cloudflare-specific variables were found in the source â€” not added to `.env.example` (would be noise without a code reference).
...
- **Group 1: Batch-container relationship** (5 tests) â€” cross-table consistency between `cv_container_state.current_batch_id` and `cv_plant_assignments`, including active/empty/teardown/ready state invariants
- **Group 2: Recipe versioning** (5 tests) â€” API rejects duplicate active recipe name (409), old version deactivated on new version creation, at most one active per name after versioning, at most one NULL effective_to in batch_stage_recipes, recipe ingredient FK integrity
- **Group 3: Plant assignment** (5 tests) â€” unassigned_at set after loss, unassigned_at + reason=harvested after final harvest, newly placed assignment has metrc_plant_tag IS NULL, tagged_at set after tag assignment, duplicate tag rejected and DB invariant verified
- **Group 4: Harvest integrity** (5 tests) â€” harvest events reference valid harvest_batches (cross-table), final harvest leaves container in teardown + assignment unassigned, harvest batches rejected unless batch is harvesting, referential integrity of harvest_batch.batch_id, initial status is in_progress
- **Group 5: Location/phase history** (8 tests) â€” API-created batch has exactly 1 phase_history and 1 location_history entry, initial phase_history from_status IS NULL, current_location_id matches latest location_history to_location_id (at creation and after transition), status transition adds phase_history entry, fixture-created batches also have history entries
...
- Avoided duplicating coverage from `regulatory-gaps.test.ts` (METRC tag uniqueness, plant_count_current) and `harvest.test.ts` (containerâ†’teardown on final harvest) â€” focused on cross-table consistency angles not already covered
- SQL string literals in parameterized queries use `?` placeholders, not inline quoted values â€” fixed one quoting bug caught by the test runner

### Files Modified/Created
- `src/tests/integration/data-integrity.test.ts` (new â€” 315 lines, 28 tests)
...
### What Was Done
- Updated "Current state" line: corrected Option A â†’ Option B integration, updated "features 1â€“16" â†’ "all Phase 1 features (1â€“26) complete"
...
- Marked all Phase 1 features 1â€“26 in Application Surface with âœ…
- Fixed `plant_assignments` schema: `assigned_at`/`assigned_by` â†’ `placed_at`/`placed_by` (migration 014 renamed these)
...
### Key Decisions
- Did not remove any content that is still accurate (all original spec, business rules, UX requirements, field rules â€” all still valid)
- Added clarifications inline rather than overwriting sections
- The Sibling App "Resolution Patterns" (Options Aâ€“D) section was left intact as historical context but the intro paragraph was replaced with a settled-integration summary

### Files Modified/Created
- `CLAUDE.md` â€” updated throughout (+269 lines, -44 lines)
- `.claude/session_context.md` â€” appended this entry
...
- `src/tests/integration/data-integrity.test.ts` was committed by a prior Felix session (315 tests); the test count in CLAUDE.md still says 287 â€” that will auto-update as tasks proceed
...
### What Was Done
- Audited all 10 tables with `photo_urls TEXT` columns across migrations 003â€“009
- Confirmed no upload endpoint, no file storage library, no client-side photo code exists anywhere
- `photo_urls` is always NULL in all rows today â€” the column is a pure placeholder
...
- Key structure: `photos/{YYYY}/{MM}/{record-type}/{record-id}/{uuid}.{ext}` â€” context-encoded for future audit tooling

### Files Modified/Created
- `docs/photo-management-design.md` (new â€” design document only, no code changes)

### Notes for Next Tasks
- Before implementing photo upload: decide on HEIC transcoding approach (iOS default format â€” needs either client-side polyfill or server-side `sharp` transcode)
- Before implementing photo upload: define the "wrong photo voiding" workflow â€” the `corrects_id` append-only pattern needs an equivalent for photo metadata
- Offline photo queue (base64 in localStorage â†’ sync on reconnect) should be scoped in Phase 2 alongside presigned URLs
...
## Task: MDA report fixes â€” time and lot number (HIGH-01/02)
**Completed:** 2026-05-22

### What Was Done
- **HIGH-01**: Renamed `application_date` â†’ `application_datetime` in the MDA pesticide report output map and CSV columns array; removed `.slice(0, 10)` so the full ISO timestamp is preserved per MN Statute 18B.37's time-of-day requirement.
...
### Key Decisions
- Field rename from `application_date` to `application_datetime` is a breaking change to the MDA export API surface â€” any consumer (frontend MdaReport page) that reads `application_date` will need to be updated if it displays that field by key name. The field was previously date-only so this is strictly an improvement.
- `lot_number` comes directly from the `cv_input_lots` JOIN â€” no farmstock API call needed since lot data lives in the local DB.

### Files Modified/Created
- `src/api/routes/exports.ts` â€” MDA pesticide handler only (3 edits)

### Notes for Next Tasks
- The MdaReport frontend page (`client/src/pages/reports/MdaReport.jsx`) renders the MDA export â€” check whether it accesses `application_date` by key name and update to `application_datetime` if so.
...
- In `src/api/routes/harvest.ts`, `POST /batches/:harvestBatchId/events`:
  - Changed `SELECT status FROM cv_batches` â†’ `SELECT status, metrc_plant_batch_uid FROM cv_batches`
...
- Gate placed after the `plantBatch` null check, before the status checks â€” consistent with the order specified in the task instructions and logically correct.

### Files Modified/Created
- `src/api/routes/harvest.ts` â€” METRC UID gate added (lines 233â€“241)
- `src/tests/helpers/fixtures.ts` â€” `createTestBatch` updated with default UID
...
### What Was Done
- Created `src/db/migrations/019_indexes.ts` with all 29 missing indexes from the schema performance audit (Section 6 of `docs/audit-schema-performance.md`). Note: numbering is 019, not 015 â€” migrations skip 015 (go 014 â†’ 016 â†’ 017 â†’ 018 â†’ 019).
...
### Files Modified/Created
- `src/db/migrations/019_indexes.ts` â€” new file (234 lines, 29 indexes + PRAGMA in up/down)
- `src/db/index.ts` â€” 5 PRAGMA lines added after existing pragmas

### Notes for Next Tasks
- `npx tsc --noEmit` passes clean.
- Test failures (176 failing) are the pre-existing `app is undefined in afterAll` infrastructure issue documented in CLAUDE.md â€” not caused by this change.
...
- NavBar already has 7 items (Today, Scan, Batches, Apply, Observe, Containers, Locations) â€” skipped per task instructions.

### Key Decisions
- `/inputs` and `/rei` were already linked in other ApplicationsHub sections ("Recipes & Catalog" and "Observations & Safety"), but the unified `/recipes` route (RecipeIndex â€” both fertigation + foliar together) was not. The new section adds all three explicitly under a "Library & Reference" heading for discoverability.
- No duplication removed from existing sections to avoid churn.

### Files Modified/Created
- `client/src/pages/applications/ApplicationsHub.jsx` â€” 41 lines added (new Library & Reference section)
...
### What Was Done
- Created `src/db/migrations/020_updated_at.ts` â€” adds `updated_at TEXT` column to cv_applications_fertigation, cv_applications_foliar, cv_applications_pesticide, cv_container_amendments, cv_observations, cv_plant_loss_events; backfills existing rows with `created_at` value
...
- cv_plant_loss_events has no PATCH route â€” no route change needed
- `npx tsc --noEmit` passes; test counts unchanged from baseline (pre-existing infra failures, not caused by this change)

### Key Decisions
- Used `datetime('now')` as a SQL expression in the updates array rather than a `?` parameter â€” cleaner, no value array mismatch possible
...
### Files Modified/Created
- `src/db/migrations/020_updated_at.ts` â€” new migration
- `src/api/routes/fertigation-applications.ts` â€” PATCH updated_at
- `src/api/routes/foliar-applications.ts` â€” PATCH updated_at
- `src/api/routes/pesticide-applications.ts` â€” PATCH updated_at
- `src/api/routes/container-amendments.ts` â€” PATCH updated_at
- `src/api/routes/observations.ts` â€” PATCH updated_at
...
### Key Decisions
- Used a LEFT JOIN to the latest teardown event (MAX teardown_id subquery) to correctly identify teardown containers with no soil sample â€” handles the case where no teardown event exists yet.
...
### Files Modified/Created
- `src/api/routes/exports.ts` â€” added `GET /pending-actions` route (43 lines added before closing `};`)
- `client/src/api.js` â€” added `getPendingActions` method
- `client/src/pages/Today.jsx` â€” added state, data fetch, and `PendingActionsSection` component

### Notes for Next Tasks
- `GET /containers?state=teardown` and `?state=startup` filter params already supported by the existing containers route and ContainerDashboard â€” the links work correctly.
...
### What Was Done
- Created `client/src/pages/harvest/WasteTrimList.jsx` â€” new page at `/harvest/waste-trim?batch_id=X`
- Records are loaded via `api.getWasteTrim({ batch_id })` and grouped by `waste_status` (collected â†’ held â†’ disposed â†’ reported)
...
- No `held` state transitions implemented â€” the PATCH endpoint only sets `disposed`; `held` is shown as actionable (can be marked disposed) since there's no dedicated "hold" UI action yet
...
### Notes for Next Tasks
- The `held` state has no "mark held" UI action â€” records can only enter `held` via direct DB or future UI; current UI only exposes the `collected â†’ disposed` transition
...
- `/harvest/waste-trim` without `batch_id` will show all records across all batches (no batch filter) â€” useful for admin; could add a batch selector dropdown if needed
...
- Also queries `cv_batch_location_history` joined with `cv_locations` to get the location name active at each transition time (linear scan â€” finds the latest location move at or before each transition's timestamp)
...
- Supports `?format=csv` â€” CSV download with filename `metrc-phases-{batchId}-{date}.csv`
...
### Key Decisions
- Location is resolved in JavaScript (linear scan per phase row) rather than a correlated subquery â€” simpler and the dataset is small (â‰¤20 transitions per batch)
...
### Files Modified/Created
- `src/api/routes/exports.ts` â€” `toMetrcPhase` added to import + `MetrcPhasesQuerySchema` + route handler (73 lines added)
- `client/src/api.js` â€” 2 new API methods added

### Notes for Next Tasks
- `npx tsc --noEmit` passes clean; committed 4065f58 and pushed to origin/master
- No frontend page built for this export yet â€” operators can call the endpoint directly or use `api.downloadMetrcPhasesCsv(batchId)` via browser tools or a future UI
...
- `down()` uses `dropColumn` (valid for simple column additions in SQLite via Knex) rather than the table-swap pattern used in migration 014 â€” adding columns is reversible.

### Files Modified/Created
- `src/db/migrations/021_tag_assignment_sync.ts` â€” new migration
- `src/api/routes/exports.ts` â€” added `MetrcTagAssignmentsQuerySchema` + route handler
- `client/src/api.js` â€” added two API client methods
...
- The export endpoint is wired but the tag-assignment sync flow (marking synced after manual METRC entry) is not yet implemented â€” that would be a PATCH `/api/tag-assignments/:id/sync-status` endpoint.
- `metrc_reconciliation` endpoint in exports.ts does not yet include tag assignment sync counts â€” a follow-up task should add that panel.
...
- Supports `?format=csv` (one row per harvest_batch Ã— product_type)
- Added `getMetrcHarvestExport(batchId, params)` and `downloadMetrcHarvestCsv(batchId)` to `client/src/api.js`

### Key Decisions
- `cv_harvest_batches` has no `metrc_sync_status` column â€” computed it from `cv_plant_harvest_events` via a correlated subquery (failed > pending > synced > not_required precedence)
...
### Files Modified/Created
- `src/api/routes/exports.ts` â€” added `/metrc-harvest/:batchId` route; added `makeHarvestBatchName` to import
- `client/src/api.js` â€” added `getMetrcHarvestExport` and `downloadMetrcHarvestCsv`
...
- **SubZoneFieldMap.jsx** (`client/src/pages/containers/SubZoneFieldMap.jsx`): New page at `/containers/map/:subZoneId`. Loads containers via api.getContainers({ sub_zone_id }). Shows: header with sub-zone ID, pot size, state count chips, REI/obs alert badges; color-coded container grid (5 rows, horizontally scrollable strips); red dot overlay for active REI, amber dot for open observations; tap â†’ /containers/:containerId; 300ms long-press â†’ QuickActionSheet with links to Observe, Foliar, Pesticide, Record Loss.
- **App.jsx**: Added import + route `/containers/map/:subZoneId` (no minRole).
- **ContainerDashboard.jsx**: Added `Link` import, added "Field Map â†’" link in each sub-zone card footer alongside "View Grid â†’".
- **LocationView.jsx**: Added `Link` import, added "Field Map â†’" link in desktop FieldSubZoneCard header.
...
### Key Decisions
- Used correlated subqueries in the SQL SELECT rather than a post-query loop â€” cleaner and SQLite handles them fine at this data scale (max ~300 containers per sub-zone).
...
- Did not modify the `api.getContainers()` signature â€” it already accepts arbitrary params and passes them as query strings. The new fields come back in the response automatically.
- State color palette in SubZoneFieldMap matches the task spec exactly (green-700, yellow-500, orange-500, blue-500, green-200, gray-400) â€” note this differs slightly from ContainerDashboard's palette for intentional visual differentiation.

### Files Modified/Created
- `src/api/routes/containers.ts` â€” added has_open_observation + rei_active_until subqueries
- `client/src/pages/containers/SubZoneFieldMap.jsx` â€” new file
- `client/src/App.jsx` â€” import + route
- `client/src/pages/containers/ContainerDashboard.jsx` â€” Link import + Field Map link per card
- `client/src/pages/locations/LocationView.jsx` â€” Link import + Field Map link per sub-zone card
- `client/src/api.js` â€” documentation comment

### Notes for Next Tasks
- The quick-action sheet in SubZoneFieldMap uses query params (?container_id=...) to pre-fill forms â€” verify FoliarNew, PesticideNew, ObservationNew read these params if they haven't been wired yet.
- SubZoneFieldMap row strips are horizontally scrollable â€” on tablet in landscape this should display well; no special tablet layout needed for now.
...
- Three status modes:
  - `awaiting_collection`: containers in `current_state='teardown'` where the most recent teardown has `soil_sample_collected=0`; joins cv_container_state â†’ cv_containers â†’ cv_rows for sub_zone_id
...
- Created `client/src/pages/containers/SoilSampleTracker.jsx` at route `/soil-samples`:
  ...
  - Results In: pH/EC chips colored by interpretation (deficientâ†’red, optimalâ†’green, etc.)
  - All rows navigate to `/containers/:containerId` on tap
- Added "Soil Samples â†’" link to ContainerDashboard header (all-zones view)
...
- `days_waiting` computed in SQL via `julianday('now') - julianday(lab_sent_at)` cast to INTEGER â€” avoids JS date math on string timestamps
...
### Files Modified/Created
- `src/api/routes/container-lifecycle.ts` â€” `soilSamplesTrackerRoutes` named export added (85 lines)
- `src/api/app.ts` â€” import updated + `soilSamplesTrackerRoutes` registered at `/api/soil-samples`
- `client/src/api.js` â€” `getGlobalSoilSamples` method added
- `client/src/pages/containers/SoilSampleTracker.jsx` â€” new file (230 lines)
- `client/src/App.jsx` â€” import + `/soil-samples` route
- `client/src/pages/containers/ContainerDashboard.jsx` â€” "Soil Samples â†’" link in header

### Notes for Next Tasks
- `npx tsc --noEmit` passes clean; `npm run build` passes clean; chunk size warning is pre-existing
- The tracker links to ContainerDetail for entering results â€” SoilSampleForm is reachable from ContainerDetail when state is 'teardown'
- `lab_sent_at` field is set on cv_soil_samples but there's no PATCH endpoint to update it after collection â€” operators would need a UI to mark "sent to lab" if that workflow is needed
...
### What Was Done
- **GET /api/batches/:id** â€” added `teardown_eligible_count` field: COUNT of containers in `cv_container_state` where `current_batch_id = batch_id AND current_state IN ('active', 'empty')`. Included in every GET /:id response so the frontend can display the count in the confirmation modal without an extra round-trip.
- **POST /api/batches/:id/bulk-teardown** â€” supervisor-only route. Validates batch is `closed` (or `harvesting` with no active plant assignments). Runs a single transaction that: (1) INSERTs a `cv_teardown_events` row per eligible container, (2) UPDATEs `cv_container_state` to `teardown`, (3) INSERTs `cv_container_state_transitions` with `trigger_event='batch_closed'`. Returns `{ transitioned_count, teardown_ids[] }`.
- **client/src/api.js** â€” added `bulkTeardown(batchId)` method.
- **BatchDetail.jsx** â€” when `batch.status === 'closed'` and `isSupervisor` and `teardown_eligible_count > 0`: shows amber "Start Teardown for All Containers (N)" button. Button opens `BulkTeardownModal` (new bottom-sheet sub-component) showing the count, a clarifying note that individual checklist items still need completion, and Cancel/Confirm buttons. On confirm: calls `api.bulkTeardown`, shows "Teardown started for N containers âœ“" toast, refreshes batch.
- **src/tests/integration/batches.test.ts** â€” 6 new integration tests: transitions active+empty containers, creates teardown_events, creates state transition log entries, returns count=0 when no eligible containers, rejects non-closed batch (400), rejects grower role (403).

### Key Decisions
- `teardown_eligible_count` added to GET /:id response rather than requiring a separate pre-fetch or dry-run mode â€” the count is always relevant for closed batches and cheap to compute.
- The modal button text mirrors the count from the batch response (no spinner/pre-fetch click pattern needed).
- Route returns HTTP 200 (not 201) for the zero-eligible case â€” no record was created, nothing to 201 about.
- Each container gets its own `cv_teardown_events` row (matching the single-container teardown pattern) â€” operators must still complete the per-container checklists (plant removal, cleaning, soil sample) individually via the container record.

### Files Modified/Created
- `src/api/routes/batches.ts` â€” `teardown_eligible_count` in GET /:id + `POST /:id/bulk-teardown` route
- `client/src/api.js` â€” `bulkTeardown` method
- `client/src/pages/batches/BatchDetail.jsx` â€” state, handleBulkTeardown, button, BulkTeardownModal sub-component
- `src/tests/integration/batches.test.ts` â€” 6 new tests (advanceBatchTo + putContainerActive/Empty imports added)

### Notes for Next Tasks
- 321 tests passing (up from 315); `npx tsc --noEmit` clean; committed 5270970 and pushed to origin/master
- The `teardown_eligible_count` field is included in every GET /batches/:id response for all batch statuses â€” it will be 0 for non-closed batches since their containers won't have `current_batch_id` pointing to them in teardown-eligible states
- Bulk Startup (transitioning teardown â†’ startup for a batch) is a natural follow-on; not built yet
...
### What Was Done
- Added `BulkScanOverlay` component to `TagAssignmentWalkthrough.jsx` â€” a full-screen camera scan-loop for fast batch tagging
- Scan Mode flow: scan container QR â†’ app identifies untagged placement â†’ shows container ID â†’ operator scans/types 24-char METRC tag â†’ auto-submits â†’ green flash â†’ loops back to scan next container
- 409 conflict shows existing `ReassignModal`; torch toggle, manual container ID fallback, and "Exit Scan Mode" button included
- Stable `tick` callback with no deps (uses mutable refs for fresh rows/completedIds data) â€” avoids stale closure issues in the RAF loop
...
- Success flash is 700ms then auto-returns to scan-container step â€” no user tap needed between containers
- `jsQR` already a dependency (used by ContainerScanner); no new packages needed

### Files Modified/Created
- `client/src/pages/containers/TagAssignmentWalkthrough.jsx` â€” added `BulkScanOverlay`, `completedIds` state, `scanMode` state, `exitScanMode`
- `client/src/pages/batches/BatchDetail.jsx` â€” added "Bulk Scan Mode" Link in quick actions section
- `src/api/routes/batches.ts` â€” added `untagged_count` query to GET /:id response
...
### Key Decisions
- `cv_plant_assignments` has no `updated_at` column â€” only `container_id` is updated (task spec included `updated_at` but that column doesn't exist on this table; per migration 014 schema)
- Destination trigger_event is `'plant_replaced'` for both destination activation and source emptying â€” closest semantic match in the existing enum
- ScanOverlay is a self-contained inline component (not a separate page/route) â€” avoids needing a callback URL pattern; same jsQR approach as ContainerScanner
...
- `client/src/pages/containers/PlantMoveForm.jsx` (new â€” 290 lines)
...
- The move route does NOT create a METRC sync event â€” moves within a batch do not require METRC notification in Phase 1 (Phase 4 will add this)
...
### What Was Done
- Added `GET /api/tag-assignments` list endpoint to `src/api/routes/tag-assignments.ts`. Returns all active (unassigned_at IS NULL) plant assignments with optional `?batch_id` and `?sub_zone_id` filters. Sorted sub_zone â†’ row_number â†’ container position. Response: `{ total, assignments[] }` with the full ASSIGNMENT_SELECT enrichment (strain_name, row_id, sub_zone_id, container_state, etc.).
- Added `api.getTagAssignments(params)` method to `client/src/api.js`.
- Created `client/src/pages/containers/AuditMode.jsx` â€” 3-step guided audit workflow at `/audit` (minRole=supervisor):
  ...
  - **Step 2 (Walk)**: Loads tagged assignments via `getTagAssignments`. For each container: shows container_id, strain, and last 4 digits of METRC tag in large display (5xl). Three tap targets: "Verified âœ“" (green), "Tag Missing âœ—" (red), "Mismatch âš " (amber). Auto-advances to next unreviewed container on tap. Progress bar + navigation buttons. Finish Audit button available at any point.
  - **Step 3 (Report)**: Summary 2Ã—2 grid (verified / missing / mismatch / not reviewed). Discrepancy list showing container_id, strain, tag last-4, and MISSING/MISMATCH badge. "â†“ Export CSV" button generates a downloadable CSV. "New Audit" resets to step 1.
...
- Container state is held in the root `AuditMode` component and passed down to child steps â€” this allows the report step to access the full container list from the walk step without re-fetching.
...
- CSV export is client-side (Blob â†’ URL.createObjectURL) â€” no server round-trip needed.
...
### Files Modified/Created
- `src/api/routes/tag-assignments.ts` â€” `GET /` list endpoint added (27 lines)
- `client/src/api.js` â€” `getTagAssignments` method added
- `client/src/pages/containers/AuditMode.jsx` â€” new file (~310 lines)
- `client/src/App.jsx` â€” import + `/audit` route (minRole=supervisor)
- `client/src/pages/applications/ApplicationsHub.jsx` â€” Compliance section with Tag Audit link

### Notes for Next Tasks
- `npx tsc --noEmit` passes clean; `npm run build` passes clean; committed 7a1c9ca and pushed to origin/master
- The audit does not persist results to the database â€” it is a session-only workflow. If persistence is needed in the future, a `cv_audit_sessions` table would capture the outcome per assignment.
...
### Key Decisions
- For recipe versioning (`isVersioning=true`), draft is restored INSIDE the API `.then()` callback after API data is loaded, so draft values override API baseline values â€” preserves in-progress edits across interruptions
...
### Files Modified/Created
- `client/src/pages/recipes/FertigationRecipeEdit.jsx` â€” added saveDraft + debounce + restore
- `client/src/pages/recipes/FoliarRecipeEdit.jsx` â€” added saveDraft + debounce + restore
- `client/src/pages/containers/PlantReplacementForm.jsx` â€” added saveDraft + debounce + restore
...
## Task: PDF cultivation record â€” browser print (HIGH-08)
**Completed:** 2026-05-22
...
- Footer: Cultivate Â· Fairwater Farm Â· MN Statute 342.25, export timestamp CT
...
- Pesticide section is the widest table (11 columns) â€” font-size 8.5px handles it without horizontal scroll on letter paper
- `phi_compliant` is stored as SQLite INTEGER 0/1; checks `=== 1` and `=== 0` rather than truthiness

### Files Modified/Created
- `client/src/pages/exports/CultivationRecord.jsx` â€” added ~273 lines of print infrastructure + button
...
- The print window does not require pop-up permissions on Safari iOS (uses synchronous write), but desktop Chrome/Firefox will block if pop-ups are disabled â€” the existing alert() handles that

---

## Task: Inspection Mode â€” row-walk swipe navigation (P2-02)
**Completed:** 2026-05-22

### What Was Done
- Created `client/src/pages/containers/InspectionMode.jsx` â€” full-screen overlay at `/inspect/:rowId` (e.g. `/inspect/Z1-A-R3`)
...
- Prev/Next arrow buttons with position label (C5 â†’ / â† C4)
- Harvest Readiness Panel: shown only when `batch_status === 'harvest_window'`; maturity % slider (0â€“100, step 5), Ready/Not Ready toggle (full-width, 56pt tap targets), priority picker (1â€“5 chips); POSTs to `api.createObservation({ category: 'harvest_readiness', maturity_pct, ready_to_harvest, harvest_priority, note })`; draft persisted to `cv_draft_readiness_{containerId}`; invalidates obs cache after save
...
- Added "Inspect Row â†’" buttons to `SubZoneFieldMap.jsx` row headers (teal text, navigates to `/inspect/Z${zone}-${desig}-R${rn}`)
- Added "Inspect Rows" teal section to `BatchDetail.jsx` for field-stage batches (field-veg, field-flower, flush, harvest_window, harvesting) â€” 5 row buttons (R1â€“R5) derived from `batch.sub_zone_id`
...
- `parseRowId('Z1-A-R3')` â†’ `{ subZoneId: 'Z1A', rowNumber: 3 }` â€” filters full sub-zone container list by `row_number` field
...
### Files Modified/Created
- `client/src/pages/containers/InspectionMode.jsx` (new â€” 330 lines)
- `client/src/App.jsx` (import + route `/inspect/:rowId`)
- `client/src/pages/containers/SubZoneFieldMap.jsx` (row header: teal "Inspect Row â†’" button)
...
- `return_to` query param on action buttons is wired but not consumed by the destination forms â€” any form that wants "back to inspection mode" behavior would need to check `searchParams.get('return_to')` on successful save
...
### What Was Done
- Created `src/api/routes/analytics.ts` with `GET /api/analytics/applicators` â€” UNION ALL query across all four application tables (cv_applications_fertigation, cv_applications_foliar, cv_applications_pesticide, cv_container_amendments) with per-applicator aggregation: application_count, fertigation_count, pesticide_count, avg_ec_deviation (ABS of measured vs recipe midpoint), first/last application timestamps
- Registered analytics routes in `src/api/app.ts` at prefix `/api/analytics`
- Added `getApplicatorMetrics()` to `client/src/api.js`
- Created `client/src/pages/analytics/ApplicatorMetrics.jsx` â€” table with date range filter, EC deviation color-coded (amber if >0.2), pesticide count highlighted red
...
### Key Decisions
- Used CTE with UNION ALL for the four-table aggregation â€” cleanest SQL for this cross-table aggregate; no N+1 risk
...
### Key Decisions
- Used `COALESCE(MAX(product_name_snapshot), 'Product #' || input_id)` â€” the aggregate `MAX()` satisfies SQLite's GROUP BY requirement while picking the snapshot value when present.
...
### Files Modified/Created
- `src/api/routes/analytics.ts` â€” pesticide-summary endpoint added
- `client/src/api.js` â€” getPesticideSummary method added
- `client/src/pages/analytics/PesticideSummary.jsx` â€” new file
- `client/src/App.jsx` â€” import + route registered
- `client/src/pages/applications/ApplicationsHub.jsx` â€” Annual Pesticide Summary link in Analytics section

### Notes for Next Tasks
- The endpoint returns `{ year, products: [...] }` envelope â€” the `year` field echoes the resolved year for the client to display.
- Print stylesheet is injected via a `<style>` tag in the JSX component â€” not a shared CSS module. If multiple pages need the same print styles, extract to a shared print.css.
...
- Created `client/src/pages/analytics/AnnualTracker.jsx` â€” CSS Grid Gantt chart at `/analytics/annual`:
  - X axis: 52 calendar weeks (17px/week = 884px total, scrollable)
  - Y axis: 8 sub-zones (Z1Aâ€“Z4B) as fixed rows
  ...
  - Click bar â†’ navigate to /batches/:id
  ...
  - Year picker (â€¹/â€º) â€” no upper bound, empty chart for years with no data
...
- Tooltip uses `position: fixed` + clientX/clientY â€” works correctly across the horizontal scroll container.
- Chunk size warning is pre-existing (QR scanner session); not caused by this change.

### Files Modified/Created
- `src/api/routes/analytics.ts` â€” annual-tracker route added
- `client/src/api.js` â€” getAnnualTracker method added
- `client/src/pages/analytics/AnnualTracker.jsx` â€” new
- `client/src/App.jsx` â€” import + /analytics/annual route
- `client/src/pages/applications/ApplicationsHub.jsx` â€” Analytics section entry added

### Notes for Next Tasks
- `npx tsc --noEmit` passes clean; `npm run build` passes (6.96s, 250KB gzipped)
- Committed and pushed as cffe592
- The Gantt chart has no test coverage â€” it's a pure UI component; business logic lives in the backend query
...
- Weight unit conversion factors: oz â†’ g = Ã— 28.3495, lb â†’ g = Ã— 453.592; fallback passes `wet_weight` as-is for unknown units.
...
### Files Modified/Created
- `src/api/routes/analytics.ts` â€” recipe-performance endpoint added (66 lines)
- `client/src/api.js` â€” getRecipePerformance method added
- `client/src/pages/analytics/RecipePerformance.jsx` â€” new file (130 lines)
- `client/src/App.jsx` â€” import + /analytics/recipe-performance route
- `client/src/pages/applications/ApplicationsHub.jsx` â€” Recipe Performance link in Analytics section

### Notes for Next Tasks
- `npx tsc --noEmit` passes clean; `npm run build` passes; committed and pushed as 0ff016b
- Results will be empty until at least one batch reaches final_harvest status â€” expected for a new operation
- Phase 3 analytics still pending: EC/pH trend charts (recharts), cross-batch strain Ã— sub-zone comparisons
...
- Best/worst highlighting only fires when â‰¥2 non-null values exist for a metric; direction=null metrics (strain, date, status) are never highlighted.
- `getBatches({ limit: 50 })` is used for the selector â€” handles the expected dataset size (typically <20 batches/year).

### Files Modified/Created
- `src/api/routes/analytics.ts` â€” added `/compare` handler
- `client/src/api.js` â€” added `getCrossBatchCompare`
- `client/src/pages/analytics/CrossBatchCompare.jsx` (new)
- `client/src/App.jsx` â€” import + route
- `client/src/pages/applications/ApplicationsHub.jsx` â€” Analytics section link
...
- Batch selector shows `batch_name` if present, falling back to `#batch_id` â€” depends on whether batches API returns `batch_name` field.
...
### What Was Done
- Created `client/src/lib/offlineQueue.js` â€” IndexedDB-backed offline write queue using the `idb` library (already installed). Exports:
  - `enqueueWrite({ endpoint, payload, entity_type, method })` â€” adds a record to `pending_writes` store in `cultivate-offline` IndexedDB
  - `flushQueue()` â€” reads all pending writes in creation order, POSTs each to the API with the stored auth token, removes on 2xx, marks as `failed` on 4xx (permanent failure), increments retry_count on 5xx/network error; max 3 retries before marking failed
  - `getQueueDepth()` â€” returns count of `pending` (not `failed`) items
  - `useOfflineSubmit({ draftKey, onSuccess, onError })` â€” hook that wraps any form save; tries the API call, on network error writes to IndexedDB and calls `onSuccess(null, true)` (isOffline=true); clears draftKey on both success paths
  - `useSyncQueue()` â€” hook for App.jsx root; calls flushQueue on mount and on window `online` event
  - `useSyncStatus()` â€” hook for NavBar; polls queue every 5s and on online/offline events; returns `{ pending, failed }`
- Wired `useOfflineSubmit` into all 7 application entry forms:
  1. FertigationNew.jsx
  2. FoliarNew.jsx
  3. AmendmentNew.jsx (also gained first-time offline support â€” previously had no offline handling)
  4. PesticideNew.jsx
  5. FinalHarvestForm.jsx
  6. PartialHarvestForm.jsx
  7. PlantLossForm.jsx
- Compliance-critical forms (FinalHarvestForm, PesticideNew) show a prominent amber border banner (`âš  ... PENDING SYNC. Do NOT re-enter...`) instead of the generic gray indicator
- `NavBar.jsx`: Added `SyncBadge` component overlaid on the Today nav item â€” green dot (all synced), amber dot + count (N pending), red `!` dot (any failed)
- `App.jsx`: Added `useSyncQueue()` call at the root component level

### Key Decisions
- `useOfflineSubmit` takes `draftKey` at hook init and `submitFn`/`queueEntry` at call time (not at init) â€” this avoids stale closure issues since payload is built fresh inside `handleSave` on each call
- Callbacks (`onSuccess`, `onError`) stored in a ref internally so forms don't need `useCallback`; always calls the latest version
- Offline draft is cleared on both success paths (online save and offline queue) â€” the data is now in either the server DB or IndexedDB, not in localStorage
- `pending_writes` store uses auto-generated ID (`${Date.now()}-${random}`) keyed as `keyPath: 'id'` â€” no auto-increment needed since we only read/delete by ID
- `flushQueue` runs on mount in `useSyncQueue` â€” catches any writes made while offline that persisted across page reloads

### Files Modified/Created
- `client/src/lib/offlineQueue.js` â€” new file (186 lines)
- `client/src/components/NavBar.jsx` â€” SyncBadge component added
- `client/src/App.jsx` â€” useSyncQueue import + call
- `client/src/pages/applications/FertigationNew.jsx` â€” useOfflineSubmit wired
- `client/src/pages/applications/FoliarNew.jsx` â€” useOfflineSubmit wired
- `client/src/pages/applications/PesticideNew.jsx` â€” useOfflineSubmit wired, prominent banner
- `client/src/pages/containers/AmendmentNew.jsx` â€” useOfflineSubmit wired (new offline support)
- `client/src/pages/harvest/FinalHarvestForm.jsx` â€” useOfflineSubmit wired, prominent banner
- `client/src/pages/harvest/PartialHarvestForm.jsx` â€” useOfflineSubmit wired
- `client/src/pages/containers/PlantLossForm.jsx` â€” useOfflineSubmit wired

### Notes for Next Tasks
- `npm run build` passes (1062KB chunk, same pre-existing size warning)
- Committed e3b88a5 and pushed to origin/master
- `failed` queue items show a red `!` badge in the NavBar but there is no conflict resolution UI yet (roadmap item) â€” users must navigate to `/applications/*` to manually re-enter failed records
- Sync flush runs on mount + online event only. There is no 30-second interval (roadmap suggested setInterval fallback) â€” this keeps it simple; add interval if needed
- The `idb` library was already installed (`^8.0.3`) â€” no new dependencies added
...
- **DEBT-04**: Added `ContainerStateSchema`, `BulkSetStateSchema` (with `.refine()` requiring scope_id when scope â‰  all), and `ContainerNotesSchema` to `containers.ts`. Three handlers (PATCH /:id/state, POST /admin/bulk-set-state, PATCH /:id/notes) now use `.parse()`.
...
- `BulkSetStateSchema` uses Zod `.refine()` for the cross-field scope/scope_id dependency â€” cleaner than manual if-check.
...
- DEBT-07: Replaced JS aggregation of 1,180 container rows in `GET /containers/summary` with a SQL `GROUP BY` (â‰¤48 rows); same response shape
- DEBT-12: Removed `api.bulkResetContainersToReady` from `client/src/api.js` (called deprecated backend alias, unused in any page)
- DEBT-13: Changed `{ error: 'Invalid request', details: ... }` â†’ `{ error: 'Invalid request', issues: ... }` in `POST /auth/login` to match all other Zod error responses
...
### Key Decisions
- `GET /containers/summary` still does minimal JS pivot (from â‰¤48 GROUP BY rows) to build the `counts` object; result shape is unchanged for the frontend
...
- The deprecated `POST /containers/admin/bulk-reset-ready` backend route still exists in `containers.ts` â€” it could be removed in a future cleanup pass if no external callers exist
...
### What Was Done
- Added `PATCH /waste-trim/:id/hold` to `src/api/routes/harvest.ts`. Transitions `waste_status` from `collected` â†’ `held`. Validates current status is `collected` (400 if not). Updates `waste_status`, `waste_status_updated_at`, `updated_at`; optionally updates `notes`.
- Added `PATCH /waste-trim/:id/report` to `src/api/routes/harvest.ts`. Transitions `waste_status` from `disposed` â†’ `reported`. Validates current status is `disposed` (400 if not). Updates `waste_status`, `waste_status_updated_at`, `metrc_sync_status`, `metrc_synced_at` (defaults to now if not provided), `updated_at`.
...
- Updated `client/src/pages/harvest/WasteTrimList.jsx`:
  ...
  - Added `handleHold(rec)` async handler â€” calls `api.holdWasteTrim`, shows success toast, reloads list
  - Added `handleReport(rec)` async handler â€” calls `api.reportWasteTrim` with `metrc_sync_status: 'synced'`, shows success toast, reloads list
...
### Key Decisions
- Separate schemas for each endpoint (`WasteTrimHoldSchema`, `WasteTrimReportSchema`) â€” the two transitions have completely different required fields.
- `metrc_synced_at` defaults to `now` in the report handler if the client doesn't supply it â€” avoids null when callers just want to mark as synced immediately.
- `reason` field in `WasteTrimHoldSchema` is accepted but not stored (no `reason` column on the table) â€” parsed to satisfy the schema but silently ignored; only `notes` is persisted if provided.
- Action buttons rendered in a flex column `div` per record card, replacing the previous single-button inline pattern.

### Files Modified/Created
- `src/api/routes/harvest.ts` â€” two new Zod schemas + two new PATCH route handlers
- `client/src/api.js` â€” `holdWasteTrim` and `reportWasteTrim` added
- `client/src/pages/harvest/WasteTrimList.jsx` â€” `actionLoading` state, `handleHold`, `handleReport`, button layout update

### Notes for Next Tasks
- 321 tests pass; `npx tsc --noEmit` clean; committed 329b55a and pushed to origin/master
- The `reason` field is accepted by `WasteTrimHoldSchema` but not stored â€” if a hold-reason column is ever added to the table, wiring it up in the handler is a one-liner
...
- The full 4-state lifecycle is now UI-complete: collected â†’ (Mark Held) â†’ held â†’ (Mark Disposed) â†’ disposed â†’ (Mark Reported) â†’ reported
...
### Key Decisions
- Followed FertigationNew.jsx pattern exactly â€” same hook signature, same useEffect guard, same badge placement
...
### Files Modified/Created
- `client/src/pages/applications/FoliarNew.jsx` â€” sensor auto-fill added

### Notes for Next Tasks
- Build passes; committed b249229 and pushed to origin/master
- FoliarNew is now the last application entry form to receive sensor auto-fill â€” all three forms (FertigationNew, PesticideNew, FoliarNew) are now consistent
...
### Key Decisions
- Used `batch_ids: [batch.batch_id]` (array) matching the backend schema â€” the task instructions said `batch_id` (singular) but the actual API requires `batch_ids`. Did not send `applicator` from the frontend since the backend infers it from the JWT token.
- Range indicators (green/amber border) are computed as derived values in the render body, not state â€” they update live as the user types.
...
### Files Modified/Created
- `client/src/pages/batches/BatchDetail.jsx` â€” added inline fertigation panel (6 state vars, 1 handler, 2 computed vars, replaced Link with expandable div)
- `client/src/components/NavBar.jsx` â€” replaced Containers/Locations/Logout with MoreHorizontal button + MoreSheet component

### Notes for Next Tasks
- The full FertigationNew form at `/applications/fertigation/new` is still reachable from ApplicationsHub and the quick-log on Today screen â€” it handles bulk entry, timestamps, optional fields. The inline panel on BatchDetail is for the common single-batch case only.
- If a batch has no active recipe assigned, the panel shows an amber warning and disables Save â€” user must assign a recipe via the "Fertigation Recipe" card first.
...
- Created `client/src/pages/analytics/EcPhTrends.jsx` at `/analytics/batch/:batchId/trends`:
  ...
  - `computeTargetSegments()` groups adjacent rows with the same target range into one ReferenceArea per segment â€” handles recipe changes mid-batch correctly
  ...
  - X-axis tick density adapts to data count (all ticks â‰¤14 points, thinned above that)
...
- "EC / pH Trends" entry added to the Analytics section of `ApplicationsHub.jsx` (navigates to `/batches` to pick a batch â€” the trend page is reached from BatchDetail)

### Key Decisions
- `ReferenceArea` uses per-segment `x1`/`x2` (ISO string `applied_at` values matching XAxis `dataKey`) rather than a single full-width band â€” this handles mid-batch recipe changes where target EC/pH differs between recipe versions
- `computeTargetSegments()` skips segments where targets are null (no recipe joined) â€” prevents phantom bands for unapplied recipes
...
### Files Modified/Created
- `src/api/routes/analytics.ts` â€” `GET /batch/:batchId/ec-ph` endpoint added
- `client/src/api.js` â€” `getEcPhTrends` added
- `client/src/pages/analytics/EcPhTrends.jsx` â€” new file
- `client/src/App.jsx` â€” import + route added
- `client/src/pages/batches/BatchDetail.jsx` â€” EC/pH Trends link added for field stages
- `client/src/pages/applications/ApplicationsHub.jsx` â€” link added to Analytics section

### Notes for Next Tasks
- `npx tsc --noEmit` passes clean; `npm run build` passes; committed e8f867a and pushed
- The ApplicationsHub link navigates to `/batches` (pick a batch), not a direct trend URL â€” the trend page is batch-specific and accessed from BatchDetail
- `recharts` was already in `client/package.json`; no install needed
- If the batch has many recipe transitions, the ReferenceLine labels may overlap at the top of the chart â€” acceptable for now, can be mitigated with `angle` or `offset` if needed
...
### What Was Done
- Ran `npx tsc --noEmit` â€” passed with no errors
- Ran `npm test` â€” 321/321 tests passing (12 test files)
- Ran `cd client && npm run build` â€” passed; 1,440KB chunk (367KB gzipped); pre-existing chunk size warning, no new errors
...
### Notes for Next Tasks
- TypeScript: PASS â€” no errors
- Tests: 321/321 passing across 12 test files
- Build: PASS â€” chunk size warning (1,440KB / 367KB gzip) is pre-existing and acceptable per task instructions
...
- Fixed a bug in `exports.ts` where `GET /api/exports/metrc-waste` returned 500 due to referencing `wt.metrc_plant_tag` â€” a column that does not exist on `cv_plant_waste_trim_events`; fixed to use `pa.metrc_plant_tag` (from the LEFT JOIN with `cv_plant_assignments`)
...
### Files Modified/Created
- `src/tests/integration/analytics.test.ts` (new â€” 6 describe blocks, 23 tests covering all /api/analytics/* endpoints)
- `src/tests/integration/exports-new.test.ts` (new â€” 4 describe blocks, 16 tests covering metrc-phases, metrc-tag-assignments, metrc-harvest, metrc-waste)
- `src/tests/integration/lifecycle-new.test.ts` (new â€” 3 describe blocks, 19 tests covering tag-assignment move, waste-trim hold, waste-trim report)
- `src/api/routes/exports.ts` (bug fix: `wt.metrc_plant_tag` â†’ `pa.metrc_plant_tag` in metrc-waste trim query)
...
### Files Modified/Created
- `src/api/routes/exports.ts` â€” 3 new Zod schemas + 3 new route handlers (~280 lines added)
- `client/src/api.js` â€” 6 new methods added
- `client/src/pages/exports/CropInputsReport.jsx` (new â€” 195 lines)
- `client/src/pages/exports/PlantLossReport.jsx` (new â€” 213 lines)
- `client/src/pages/exports/HarvestReport.jsx` (new â€” 325 lines)
- `client/src/App.jsx` â€” 3 imports + 3 routes
- `client/src/pages/applications/ApplicationsHub.jsx` â€” 3 new compliance report buttons

### Notes for Next Tasks
- `npx tsc --noEmit` passes clean; `npm run build` passes; committed 099c03a and pushed to master
- The crop inputs report ingredient expansion uses `rate_value * volume_gallons` for computed quantity â€” works for standard rate_units; unusual units (g_per_gal etc.) will show the raw computed value which may need formatting improvement
- The harvest report collapsible batch cards show per-batch individual events when expanded â€” useful for auditor row-by-row review
- All three reports are accessible from ApplicationsHub Compliance & Reports section and are registered in App.jsx

---

## Task: OCM compliance reports â€” Reports 10, 11 (PHI Compliance, Annual Summary)
**Completed:** 2026-05-23
...
- Annual summary filters by `sow_date` year for batches; loss/harvest/waste events are filtered via batch_id IN (...) â€” avoids missing events recorded after year boundary
- Weight normalization (toGrams) added as a private function in exports.ts â€” not exported; only used by annual-summary route
...
### Files Modified/Created
- `src/api/routes/exports.ts` â€” two new routes appended before closing brace
- `client/src/api.js` â€” two new API methods
- `client/src/App.jsx` â€” two new imports + two new routes
- `client/src/pages/applications/ApplicationsHub.jsx` â€” two new entries in Compliance & Reports section
- `client/src/pages/exports/PhiComplianceReport.jsx` (new)
- `client/src/pages/exports/AnnualSummary.jsx` (new)

### Notes for Next Tasks
- npx tsc --noEmit passes clean; npm run build passes (chunk size warning is pre-existing)
- phi-compliance route uses `product_name_snapshot` from migration 018 â€” will be null for older records if snapshot wasn't captured at save time
- Annual summary by_product_type_g includes 'popcorn' as a product type (consistent with the harvest events schema) even though the task spec listed flower/larf/trim_product/other â€” the schema has popcorn so it's included
...
- **MoreSheet**: Added Planting Plans (â†’ /planting-plans) and Soil Samples (â†’ /soil-samples) to the MoreSheet items list, before the Logout button.
- **ApplicationsHub â€” Field & Containers section**: New section inserted after the 4-app type grid, before the Compliance section. Contains: Container Dashboard (â†’ /containers, Grid2x2 icon), Locations (â†’ /locations, MapPin icon), Soil Sample Tracker (â†’ /soil-samples, FlaskConical icon).
- **ApplicationsHub â€” Planning section**: New section inserted after Compliance & Reports, before Analytics. Contains: Planting Plans (â†’ /planting-plans, ClipboardList icon) with a 'Supervisor' gray pill badge.
- **ApplicationsHub â€” Admin section**: Added Strains (â†’ /strains, Sprout icon, 'Supervisor' badge) and Environmental History (â†’ /admin/environmental-history, BarChart2 icon, 'Admin' badge) to the existing Admin section.
- **EC/pH Trends fix**: Changed label from 'EC / pH Trends' to 'EC / pH Charts' and updated sub-text to 'Select a batch from the Batches list Â· per-batch EC and pH charts'. Route stays as navigate('/batches').
...
### Files Modified/Created
- `client/src/components/NavBar.jsx` â€” LayoutGrid import, Hub label/icon, MoreSheet additions
- `client/src/pages/applications/ApplicationsHub.jsx` â€” lucide-react import, Field & Containers section, Planning section, Admin additions, EC/pH fix

### Notes for Next Tasks
- npx tsc --noEmit passes clean; npm run build passes (chunk size warning is pre-existing)
- The six newly linked pages (/containers, /locations, /soil-samples, /planting-plans, /strains, /admin/environmental-history) already exist â€” this task only added entry points in the hub nav
...
- **Three new `api.js` methods**: `getMetrcTagAssignments`, `getMetrcPhaseChanges`, `downloadMetrcPhaseChangesCsv` (the other methods â€” getMetrcWasteExport, downloadMetrcWasteCsv, downloadMetrcTagAssignmentsCsv â€” already existed).
- **Four new export pages** created:
  - `MetrcHarvestExport.jsx` â€” cross-batch harvest events grouped by harvest batch with product-type subtotals; uses existing `/harvest-records` endpoint (avoids duplicate endpoint since `/metrc-harvest/:batchId` was batch-specific only)
  - `MetrcWasteExport.jsx` â€” unified waste trim + plant loss events; summary count cards; red row highlight on pending/failed sync
  - `MetrcTagAssignmentExport.jsx` â€” tag assignments grouped by batch; unsynced count badge; filter by batch ID only
  - `MetrcPhaseChangeExport.jsx` â€” phase transitions + location moves in one table; type chips (blue=phase, purple=location); amber UID missing warning
...
### Files Modified/Created
- `src/api/routes/exports.ts` â€” new `/metrc-phase-changes` route appended before closing brace
- `client/src/api.js` â€” 3 new API methods
- `client/src/App.jsx` â€” 4 new imports + 4 new routes
- `client/src/pages/applications/ApplicationsHub.jsx` â€” 4 new entries in Compliance & Reports section
...
### Notes for Next Tasks
- Commit: 71d1801 â€” `feat(exports): METRC harvest, waste, tag-assignment, phase-change frontend pages`
- npx tsc --noEmit passes clean; npm run build passes (chunk size warning is pre-existing)
...
- All 4 pages back-navigate to `/applications` (ApplicationsHub) via the â† button
...
### Key Decisions
- Selection step gated on `assignment === null && activeAssignments.length > 1` â€” clean state machine, no extra boolean flags
...
- Did NOT update ContainerDetail â€” HarvestDashboard already shows per-plant links with assignment_id; ContainerDetail can be updated separately

### Files Modified
- `client/src/pages/harvest/FinalHarvestForm.jsx` â€” only file changed
...
- Added `GET /api/applications/foliar/stage-check?input_id=X&batch_id=Y` endpoint to `foliar-applications.ts`
  ...
  - Reuses existing `getBatchStageKey` helper to map batch status â†’ stage key
  - Queries `cv_input_phi_stage_overrides` for `allowed=0` rows
  - Returns `{ blocked: true, reason }` or `{ blocked: false }`
- Updated POST handler stage-block response: 422 â†’ 400, `stage_blocked` â†’ `stage_block` for consistency
...
- Recipe mode is unaffected â€” check only runs when `mode === 'product'` and a product is selected
...
- The Partial Harvest button was left unchanged â€” it already has `assignment_id` in its URL and PartialHarvestForm doesn't have the same multi-plant selection issue

### Files Modified/Created
- `client/src/pages/containers/ContainerDetail.jsx` â€” two edits in the harvest IIFE (lines ~634â€“665)
...
### Key Decisions
- Field is optional with no client-side validation â€” matches the schema doc (nullable, can be set later)
...
### Files Modified/Created
- `src/db/migrations/022_batch_expected_harvest_date.ts` â€” new migration
- `src/api/routes/batches.ts` â€” schema + INSERT updated
- `client/src/pages/batches/BatchNew.jsx` â€” form field added

### Notes for Next Tasks
- `expected_harvest_date` is not yet in `BatchUpdateSchema` â€” PesticideNew reads it from the batch detail response, so editing it later requires a separate PATCH form update if operators need to change it post-creation
- `npx tsc --noEmit` and `npm run build` both pass clean
...
- **No suggestions based on soil sample deficiencies**: Skipped (nice-to-have per task spec; requires a parameterâ†’product mapping not yet defined).

### Files Modified/Created
- `src/api/routes/containers.ts` â€” new POST `/:id/amendments` endpoint
- `client/src/api.js` â€” `createContainerAmendmentFromStartup` added
- `client/src/pages/containers/StartupForm.jsx` â€” full rewrite with amendments section
...
### What Was Done
- Replaced 6-tab NavBar (Today Â· Scan Â· Batches Â· Hub Â· Observe Â· More) with 5-item bar: Today Â· Locations Â· Tasks Â· More Â· Logout
- Logout is a plain icon button in the primary nav bar that calls logout() directly â€” no confirmation dialog
...
- Created new Tasks page at client/src/pages/tasks/Tasks.jsx â€” worker-facing action list
...
- Added route /tasks â†’ Tasks in App.jsx

### Key Decisions
- Logout in primary bar as a plain icon (no confirmation) per spec â€” fast access without sheet friction
...
### Files Modified/Created
- client/src/components/NavBar.jsx â€” 5-item primary bar, updated MoreSheet
- client/src/pages/tasks/Tasks.jsx â€” new page (created)
- client/src/App.jsx â€” import + route /tasks
...
- Container actions section (teardown_pending, startup_pending, lab_samples_awaiting) driven by getPendingActions() shape â€” same as Today's PendingActionsSection


## Task: Locations Home screen â€” spatial location grid
**Completed:** 2026-05-23
...
- Rewrote `client/src/pages/locations/LocationView.jsx` entirely:
  ...
  - Field section: 2-col mobile / 4-col tablet grid of ZoneCard components (Zone 1â€“4)
  ...
  - Manual refresh button (â†») in page header with spin animation while loading
  - Named export remains `LocationView` (default export) â€” no App.jsx changes needed
...
### Files Modified/Created
- `src/api/routes/locations.ts` â€” new file
- `src/api/app.ts` â€” added import + register for locationsRoutes
- `client/src/api.js` â€” added getLocationsSummary()
- `client/src/pages/locations/LocationView.jsx` â€” full rewrite (retained filename/export name)
...
- No App.jsx route changes were needed â€” the file path and export name are unchanged

## Task: SubZoneFieldMap batch context + action bar + back nav
**Completed:** 2026-05-23

### What Was Done
- Fixed back navigation: replaced `<Link to="/containers">â† All Zones</Link>` with `<button onClick={() => navigate(-1)}>â† Back</button>` â€” preserves correct back destination regardless of entry path
...
- Added batch context card in header (between state count summary and REI alerts): shows strain name, status chip, day count, plant count, and "View Planting Group â†’" link; tapping card navigates to /batches/:batch_id; empty state shows italic gray text
...
### Key Decisions
- getBatches catch(() => []) wraps only the batches call, not containers â€” if containers fails the page shows an error, if only batches fails activeBatch is null and the field map still works
...
- No Pesticide quick action â€” per spec, that's a supervised action logged from ContainerDetail or the Hub

### Files Modified/Created
- `client/src/pages/containers/SubZoneFieldMap.jsx` â€” all three fixes in this one file; no backend changes

### Notes for Next Tasks
- npx tsc --noEmit passes clean; npm run build passes clean
- Action bar z-index is z-40; QuickActionSheet is z-50 â€” sheet correctly appears above action bar
...
- Created `src/db/migrations/023_location_tree.ts`:
  ...
  - Seeds existing locations with categories: Germ-01/Seedlings â†’ indoor, Cult-Hoop â†’ hoop_house, Z1Aâ€“Z4B â†’ outdoor
  - Inserts Zone 1â€“4 as outdoor parent locations (location_id 12â€“15)
  - Sets parent_location_id on Z1A/Z1Bâ†’12, Z2A/Z2Bâ†’13, Z3A/Z3Bâ†’14, Z4A/Z4Bâ†’15
  - down() uses CREATE TABLE â€¦ AS SELECT + rename pattern (SQLite constraint)
- Added `GET /api/locations/tree` to `src/api/routes/locations.ts`:
  ...
  - Assembles two-level outdoor tree (Zone parent â†’ sub-zone children) plus flat indoor/hoop_house sections
...
- Zone 1â€“4 records (location_id 12â€“15) exist in cv_locations after migration 023 runs; the frontend can use them as display groupings
...
- **Batches.jsx**: Added `useSearchParams` to read `?location_id` and `?location_name` from URL. Added `locationFilter` state. Shows a green banner "Showing batches for [name]" with âœ• to clear when a location filter is active. Passes `location_id` to `api.getBatches()` when set.
- **BatchNew.jsx**: Added `useSearchParams` to read `?location_id`. Calls `api.getLocationsTree()` to resolve the location name and sub_zone_id from the preset location. Shows a "Starting From [location]" callout when pre-set. Added Sub-Zone chip picker (8 chips: Z1Aâ€“Z4B, toggle to clear). sub_zone_id included in draft persistence and in `api.createBatch()` payload.

### Key Decisions
- `location_name` passed as URL param from LocationsHome â†’ Batches to avoid an extra API call just for the banner label.
- `getLocationsTree()` called in BatchNew for location resolution rather than adding a new single-location endpoint â€” acceptable extra call, only fires when `?location_id` is present.
...
### Notes for Next Tasks
- `AddSubLocationModal` is a named export from `LocationContextMenu.jsx` â€” reusable if needed elsewhere.
- The "Add Plant Group" action navigates to `/batches/new?location_id=X` â€” `BatchNew` form may or may not use this param to pre-select location.
...
### What Was Done
- Created `src/db/migrations/024_location_display_order.ts`: adds `display_order` (integer, default 999) and `col_span` (integer, default 1) to `cv_locations`; seeds Germ-01/Seedlings/Cult-Hoop at 10/20/10, Zone 1â€“4 at 20â€“50; conditionally sets Zone 5 to display_order=10, col_span=2 if it exists
...
- Outdoor grid in `LocationView.jsx` changed from `grid-cols-2 md:grid-cols-4` to always `grid-cols-2`; outdoor cards wrapped in a div that applies `col-span-2` when `location.col_span === 2` â€” gives Zone 5 full-width row, Zones 1+2 and 3+4 side-by-side below
- Add Location modal (`LocationView.jsx`) rebuilt as `flex flex-col` sheet with `maxHeight: 85vh`, scrollable middle section, and a sticky footer with Cancel/Save buttons that are always visible above the NavBar â€” Save is never obscured
...
- outdoor grid stays 2-column at all viewport sizes (not responsive 2â†’4) so col-span works correctly â€” a 4-col grid would need col-span-4 for full-width, but 2-col with col-span-2 is simpler and matches the spatial intent

### Files Modified/Created
- `src/db/migrations/024_location_display_order.ts` â€” new migration
- `src/api/routes/locations.ts` â€” tree sort order + admin schema + INSERT
- `client/src/pages/locations/LocationView.jsx` â€” modal rebuild + displayOrder state + outdoor grid
- `client/src/components/LocationContextMenu.jsx` â€” mobile AddSubLocationModal sheet rebuild
...
### Files Modified/Created
- `src/api/routes/locations.ts` â€” added `UpdateLocationSchema` + PATCH route
- `client/src/api.js` â€” added `updateLocation`
- `client/src/components/LocationContextMenu.jsx` â€” added Pencil import, `onEdit` prop, 'Edit Location' action
- `client/src/pages/locations/LocationView.jsx` â€” added edit modal state/effect/handler/JSX, wired `onEdit` to context menu

### Notes for Next Tasks
- The PATCH route supports `active` boolean to deactivate locations (not exposed in the modal yet â€” easy to add)
- `col_span` in the Zod schema accepts 1â€“4 but the modal only offers 1 and 2 (current operational range)
...
- Added category badge to **ZoneCard**: replaced `<h3>` + REI badge span with a `<span>` name + `<div class="flex items-center gap-1.5">` grouping a restyled âš  REI badge (text-[10px], no AlertTriangle icon) and the category badge.
- Added category badge to **NoSubZonesCard**: same pattern as IndoorCard.
- `AlertTriangle` import retained â€” still used in the global alerts banner.
...
- ZoneCard REI badge changed from `text-xs ... border ... flex items-center gap-1 <AlertTriangle>` to `text-[10px] ... bg-amber-100 âš  REI` for size parity with the new category badge.
...
- Added 5 columns to `cv_batches`: `source_type`, `seed_package_id` (FK â†’ cv_seed_packages), `seed_count_used`, `seed_weight_g`, `initial_phase`
- `down()` uses recreate-copy pattern for cv_batches (SQLite has no DROP COLUMN); drops cv_seed_packages after
- Fixed `real()` â†’ `float()` (Knex TS types don't expose `real()` on CreateTableBuilder)
...
### Notes for Next Tasks
- `cv_seed_packages.seed_count_remaining` needs to be decremented when a batch is created referencing it â€” no route does this yet
...
- LocationView.jsx: IndoorCard now checks isSeedVaultLocation(name) â€” navigates to /seed-vault instead of /batches for "Seed Vault" location; shows green border + "ðŸŒ± Seed Vault" badge and "View seed inventory â†’" text
...
- isSeedVaultLocation() checks for "seed vault" or "seed-vault" (case-insensitive) â€” covers any naming variation the operator uses
...
### Notes for Next Tasks
- seed_count_remaining is not decremented when a batch is created referencing a seed package â€” still needs wiring in batches.ts POST handler
- "Use in New Batch â†’" button navigates to /batches/new?seed_package_id=X; BatchNew pre-selects the strain but doesn't pre-fill seed_count_used or other batch fields from the package yet
...
- Field statuses (field-veg, field-flower) in initial_status still get LOCATION.GERM as starting location â€” field location is set when the planting plan is committed (same as the existing transition flow).
- `updated_at` does not exist on `cv_seed_packages` (not added by migrations 025/026), so PATCH does not set it.

### Files Modified/Created
- `src/api/routes/batches.ts` â€” BatchCreateSchema + POST handler updated
- `src/api/routes/seed-packages.ts` â€” GET query schema updated to Zod
- `client/src/api.js` â€” getSeedPackages signature updated
- `src/db/migrations/025_seed_packages_and_batch_source.ts` â€” pre-existing (unchanged)
- `src/db/migrations/026_seed_packages_enrich.ts` â€” pre-existing (unchanged)

### Notes for Next Tasks
- `cv_seed_packages` has no `updated_at` column â€” if needed, add it in a new migration.
...
## Task: BatchNew form rewrite â€” germination-first with growth phase
**Completed:** 2026-05-23

### What Was Done
- Rewrote `client/src/pages/batches/BatchNew.jsx` to be germination-first per spec:
  - Back button now uses `navigate(-1)` with label `â† Back`
  - Title changed to `New Plant Group`; subtitle is conditional on selected phase
  - Added Growth Phase chip picker (Immature / Veg / Flower) â€” default: Immature
  - Added Source Type chip picker (Seed / Clone) â€” shown only when phase=immature
  - Added Seed Package section (strain_id-filtered select + seeds-to-start + seed-weight fields + inline "Add seed package" form) â€” shown only when phase=immature AND sourceType=seed
...
### Files Modified/Created
- `client/src/pages/batches/BatchNew.jsx` â€” full rewrite (363 insertions, 143 deletions)

### Notes for Next Tasks
- BatchDetail page still shows "Plant Batch" in some places â€” may want to update to "Plant Group" for consistency
- The Batches list page title/button still says "New Plant Batch" â€” update when addressing the list page
...
- Action matrix by location type:
  - **Seed Vault**: Edit Location â†’ View Seed Packages â†’ Add Seed Package (no fertigation, no plant group)
  - **Outdoor sub-location (active batch)**: Edit â†’ Apply Fertigation â†’ Log Foliar â†’ Walk Row â†’ View Planting Group â†’ View Zone Map â†’ View History
  - **Outdoor sub-location (empty)**: Edit â†’ Add Plant Group â†’ View Zone Map â†’ View History
  - **Outdoor top-level zone**: Edit â†’ View Zone Map (first sub-zone) â†’ Add Sub-location â†’ View History
  - **Indoor / Hoop-House (active batch)**: Edit â†’ Add Plant Group â†’ Apply Fertigation â†’ View Planting Group â†’ [Add Sub-location if top-level] â†’ View History
  - **Indoor / Hoop-House (no batch)**: Edit â†’ Add Plant Group â†’ [Add Sub-location if top-level] â†’ View History
...
### Key Decisions
- `isSubLocation` is derived from `level === 'sub_location'` (the value passed by `SubZoneRow` â†’ `onOpenMenu`)
- `isSeedVault()` uses same name-matching logic already present in `LocationView.jsx`'s `isSeedVaultLocation()` â€” kept consistent but not deduplicated (different files, different scopes)
- Walk Row derives zone/designation from `sub_zone_id` (e.g. `Z1A` â†’ `/inspect/Z1-A-R1`)
...
### Files Modified
- `client/src/components/LocationContextMenu.jsx` â€” full context menu rewrite
- `client/src/pages/seed-vault/SeedVault.jsx` â€” added `?add=1` auto-open behavior

### Notes for Next Tasks
- Walk Row navigates to `/inspect/Z${zone}-${desig}-R1` â€” verify this route exists and accepts this format
...
- Created `src/lib/metrc-csv/` module scaffold:
  - `types.ts` â€” `CsvRow`, `CsvResult`, `CsvUploadType` (22 values)
  - `writer.ts` â€” `writeCsv()`: atomic write (.tmp â†’ .csv rename), UTF-8 no BOM, CRLF line endings, YYYYMMDD subdirectory, METRC_CSV_OUTPUT_DIR env var default
  - `ref-data.ts` â€” 6 typed DB lookup functions (wasteMethods, plantWasteReasons, batchWasteReasons, unitsOfMeasure, locations, strains)
  - `index.ts` â€” re-exports all public API
  - `generators/.gitkeep` â€” empty directory for future generators
- Created `src/api/routes/metrc-csv.ts` â€” Fastify plugin stub with `GET /health` (requireAuth)
- Registered in `src/api/app.ts` at prefix `/api/metrc/csv`
- Migration 042 (`042_metrc_core_tables.ts`): 9 new tables â€” cv_employees, cv_metrc_csv_uploads, cv_metrc_additive_templates, cv_metrc_additive_applications, cv_metrc_plant_state, cv_metrc_packages, cv_metrc_package_adjustments, cv_metrc_immature_waste_events, cv_metrc_immature_destruction_events
...
- `writeCsv` takes content as a pre-built CSV string â€” generators are responsible for building the header + rows
- Railway Volume must be provisioned and METRC_CSV_OUTPUT_DIR set before Phase 1 ships (C6 from integration proposal)
- `cv_metrc_plant_waste_methods`, `cv_metrc_plant_waste_reasons`, `cv_metrc_batch_waste_reasons`, `cv_metrc_units_of_measure` are empty â€” admin UI (Phase 2) populates them from MN METRC UI
...
### What Was Done
- **Generator** (`src/lib/metrc-csv/generators/additive-template.ts`): `generateAdditiveTemplateCsv(templates)` â€” emits exact 11-column header, one row per active ingredient per template, CRLF line endings, csv-safe quoting
- **Zod schemas** in `src/api/routes/metrc-csv.ts`: `ActiveIngredientSchema`, `AdditiveTemplateSchema` (with EPA-required-for-Pesticide and REI-pair-coherence refinements), `CreateAdditiveTemplatesSchema` (total ingredients â‰¤500 limit)
- **Backend routes** added to `src/api/routes/metrc-csv.ts`:
  - `POST /api/metrc/csv/additive-templates` â€” validates, dedupes names in-request + in-DB, generates CSV, writes file, transaction-inserts templates + upload log, returns `{template_ids, csv_file_path, row_count, upload_id}`
  - `GET /api/metrc/csv/additive-templates` â€” returns all templates with `active_ingredients` parsed from JSON
...
- **Tests** (`src/tests/metrc-csv/additive-template.test.ts`): 16 tests passing â€” 7 unit tests for generator (headers, CRLF, ingredient expansion, field repetition, multi-template, quoting, nulls), 7 integration tests for POST route, 2 for GET route

### Key Decisions
- Kept generator pure (no DB calls) â€” takes `AdditiveTemplateInput[]`, returns string
- `crop_input_id` on additive templates is now a plain integer (no FK enforcement) â€” the actual cross-reference to farmstock products is advisory, enforced at UI layer only, not DB layer
- Route uses `requireAuth` (grower+), not `requireRole('admin')` â€” creation is an operational task, not admin-only; admin minRole gate is on the frontend page only
...
- `src/tests/metrc-csv/additive-template.test.ts` (new â€” 16 tests)

### Notes for Next Tasks
- Migration 045 drops and recreates `cv_metrc_additive_templates` â€” any data in the table on a live DB will be preserved (INSERT...SELECT copies rows)
- The same FK-to-farmstock anti-pattern could affect other tables in 042 if `cv_metrc_additive_applications` has FKs that reference non-existent cultivate tables â€” check before building routes for those tables
- Next METRC upload type is likely #21 Plants Waste (per Phase 1 spec in integration proposal) â€” needs `cv_metrc_plant_waste_methods` and `cv_metrc_plant_waste_reasons` populated first
- The pre-existing foliar test failure (`foliar.test.ts:247` expects 422 but gets 400) is unrelated to this task â€” was failing before

---

## Task: METRC CSV: Plants Waste #21 end-to-end
**Completed:** 2026-05-26

### What Was Done
- Generator `src/lib/metrc-csv/generators/plants-waste.ts`: `PlantsWasteRow` interface, `PLANTS_WASTE_HEADERS`, `generatePlantsWasteCsv()` â€” pipe-delimited PlantLabels cell, CRLF, empty string for undefined/empty plant_labels
- Zod schemas in `metrc-csv.ts`: `PlantsWasteRowSchema` (waste_weight > 0, waste_date YYYY-MM-DD, plant_labels 24-char alphanumeric), `CreatePlantsWasteSchema` (events array min 1 max 500)
- `GET /api/metrc/csv/plants-waste/pending` â€” returns `cv_plant_waste_trim_events` where `metrc_csv_generated_at IS NULL AND waste_status IN ('collected','held')`, joined with plant_assignments, batches, strains; limit 200
- `POST /api/metrc/csv/plants-waste` â€” validates location_name against `cv_locations.metrc_name` (400 if missing), warns (not rejects) when waste method/reason ref tables empty, generates CSV, updates matching `cv_plant_waste_trim_events` (plant tag + date match via JOIN on plant_assignments), inserts `cv_metrc_csv_uploads`; returns `{csv_file_path, row_count, upload_id, warnings}`
- `client/src/pages/metrc/PlantsWasteSubmit.jsx` at `/metrc/plants-waste` â€” pending events list with checkboxes + Select all, batch METRC settings (method, unit, reason, location), inline manual entry form, Generate CSV sticky button; grower+ (no minRole gate)
- Nav link added to ApplicationsHub METRC section (between Tag Assignment and bottom)
- `getPendingPlantsWaste()` and `submitPlantsWaste()` added to `client/src/api.js`
- `src/tests/metrc-csv/plants-waste.test.ts` â€” 8 generator unit tests + 10 POST integration tests + 2 GET pending integration tests = 21 total, all pass

### Key Decisions
- Batch METRC settings (method/unit/reason) apply uniformly to all checked pending events â€” field-optimized for daily defoliation rounds where all events share the same method
- Location validation rejects unknown names rather than warning â€” a typo in location_name would silently generate a broken CSV; hard rejection forces correctness
- Plant tag update matches on `DATE(trimmed_at) = waste_date` + tag via JOIN on `cv_plant_assignments.metrc_plant_tag` â€” only updates records that haven't already been exported (`metrc_csv_generated_at IS NULL`)
- Uses `COALESCE(metrc_waste_method, ?)` so previously-set METRC fields are not overwritten

### Files Modified/Created
- `src/lib/metrc-csv/generators/plants-waste.ts` (new)
- `src/lib/metrc-csv/index.ts` (added 2 exports)
- `src/api/routes/metrc-csv.ts` (schemas + 2 new routes)
- `client/src/pages/metrc/PlantsWasteSubmit.jsx` (new)
- `client/src/api.js` (2 methods added)
- `client/src/App.jsx` (import + route)
- `client/src/pages/applications/ApplicationsHub.jsx` (nav link in METRC section)
- `src/tests/metrc-csv/plants-waste.test.ts` (new â€” 21 tests)

### Notes for Next Tasks
- `cv_metrc_plant_waste_methods` and `cv_metrc_plant_waste_reasons` reference tables start empty â€” operator must populate via admin UI (not yet built) before waste method/reason values are validated against a list
- The sticky "Generate CSV" button uses `bottom-20` to clear the NavBar (64px bottom nav) â€” consistent with the dialog bottom nav offset pattern in memory
- Pre-existing foliar test failure (`foliar.test.ts:247` â€” stage block 422 vs 400) confirmed present before and after this task; unrelated

---

## Task: METRC CSV: Reference Data Admin UI
**Completed:** 2026-05-26

### What Was Done
- Backend: 30 admin routes added to `src/api/routes/metrc-csv.ts` (all requireRole('admin'))
  - GET/POST/DELETE for waste-methods, plant-waste-reasons, batch-waste-reasons, adjustment-reasons, units-of-measure, items, sublocations
  - GET/POST for plant-tags and package-tags (bulk INSERT OR IGNORE + counts by status + last-20 recent)
  - GET/POST/PATCH for employees (activate/deactivate toggle via is_active)
- API client: 22 admin CRUD methods added to `client/src/api.js`
- Frontend: `client/src/pages/admin/MetrcSetup.jsx` (5 tabs):
  - Tab 1 Reference Data: SimpleRefSection reusable component, 6 sub-sections loaded in parallel
  - Tab 2 Sublocations: grouped by location, uses getLocationsTree() flattened to [{location_id, name}]
  - Tab 3 Tag Pools: client-side 24-char validation, import textarea, count grid, last-20 table
  - Tab 4 Employees: activate/deactivate toggle, add form
  - Tab 5 Additive Templates: link to /admin/metrc-additive-templates + inline list
- Navigation: /admin/metrc-setup route in App.jsx (minRole=admin), METRC Setup button in ApplicationsHub admin section

### Key Decisions
- Used getLocationsTree() (existing endpoint) and flattened the nested tree for the location dropdown — no new endpoint needed
- Tags uppercased on import; INSERT OR IGNORE for idempotent bulk inserts
- rowid DESC LIMIT 20 for most recently inserted tags since tag tables have no created_at column
- AdminEmployeePatchSchema.refine() ensures at least one field provided; 'field' in parsedData guard works because Zod excludes absent optional fields from output
- Sublocation duplicate check: WHERE name = ? AND (location_id IS ? OR location_id = ?) handles NULL location_id correctly (SQLite IS operator)

### Files Modified/Created
- `src/api/routes/metrc-csv.ts` (requireRole import + 7 admin schemas + ~180 lines of admin routes)
- `client/src/api.js` (22 admin methods added)
- `client/src/pages/admin/MetrcSetup.jsx` (new — ~500 lines, 5-tab admin page)
- `client/src/App.jsx` (MetrcSetup import + /admin/metrc-setup route)
- `client/src/pages/applications/ApplicationsHub.jsx` (METRC Setup button in admin section)

### Notes for Next Tasks
- npx tsc --noEmit passes clean; npm test shows 421/422 passing (1 pre-existing foliar test failure unrelated)
- Committed as feat(metrc-csv): METRC Setup admin UI — ref data, tag pools, employees (SHA: 0fdd845)
- cv_metrc_sublocations has UNIQUE(name, location_id) constraint — backend enforces this before INSERT
- Tag pools: plant tags support reserved status (for batch assignment); package tags only available|used
- Employee PATCH uses dynamic SET clause — only fields present in request body are updated

---

## Task: METRC CSV 3A: Planting family #2 #17 #18 #22
**Completed:** 2026-05-26

### What Was Done
- Created `src/lib/metrc-csv/generators/create-plantings.ts` (#2 Create Plantings): generates one CSV row per ingredient_batch_name; inserts cv_batches with status='germ', metrc_source_type='ingredient_batches'|'none'
- Created `src/lib/metrc-csv/generators/plantings-from-package.ts` (#17 Plantings from Package): validates package_label exists in cv_metrc_packages; decrements package weight_amount when adjustment provided; inserts cv_batches with metrc_source_type='package'
- Created `src/lib/metrc-csv/generators/plantings-from-plant.ts` (#18 Plantings from Plant): validates plant_label exists in cv_metrc_plant_state with is_mother_plant=1 (400 if not); inserts cv_batches with metrc_source_type='plant'; mother plant stays active
- Created `src/lib/metrc-csv/generators/split-planting.ts` (#22 Split Planting): supervisor-only; uses metrc_plant_batch_uid as CSV PlantBatch field with fallback to batch name/id; validates count <= plant_count_current
- Updated `src/lib/metrc-csv/index.ts` to export all four new generators
- Added four Zod schemas + route handlers to `src/api/routes/metrc-csv.ts`; all four use the same transaction pattern: generate CSV → writeCsv → DB side-effects → insert cv_metrc_csv_uploads

### Key Decisions
- #2 CREATE PLANTINGS: if ingredient_batch_names is empty, emits one row with IngredientPlantBatchName blank (matches template sample semantics)
- #17 package_adjustment_amount/uom are paired — refine validates both present or both absent
- #18 strain lookup for new batch uses COALESCE to fall back to first strain if strain_name not found; issues a warning instead of hard 400 since METRC may use different strain naming
- #22 SplitPlanting uses metrc_plant_batch_uid as the METRC PlantBatch identifier (required by METRC), falling back to batch.name → batch_id

### Files Modified/Created
- `src/lib/metrc-csv/generators/create-plantings.ts` (new)
- `src/lib/metrc-csv/generators/plantings-from-package.ts` (new)
- `src/lib/metrc-csv/generators/plantings-from-plant.ts` (new)
- `src/lib/metrc-csv/generators/split-planting.ts` (new)
- `src/lib/metrc-csv/index.ts` (exports added)
- `src/api/routes/metrc-csv.ts` (4 Zod schemas + 4 route handlers added)

### Notes for Next Tasks
- All four routes are POST-only; no GET/list endpoints needed per spec
- #17 and #18 strain lookup uses COALESCE fallback — if METRC strain names differ from cv_strains.name, operator will need to ensure strain names match
- Pre-existing test failure in foliar.test.ts:247 (422 vs 400 on stage_blocked) is NOT introduced by these changes; it was failing before
- Next batch: Phase 3B Destruction family (#3 Destroy Immature Plants, #4 Destroy Plants)

---

## Task: METRC CSV 3B: Destruction family #3 #4
**Completed:** 2026-05-26T00:00:00Z

### What Was Done
- Created `src/lib/metrc-csv/generators/destroy-immature.ts` — #3 Destroy Immature Plants CSV generator
  - Headers: PlantBatch,Count,WasteMethodName,WasteMaterialMixed,WasteReasonName,ReasonNote,WasteWeight,WasteUnitOfMeasure,ActualDate
  - When waste_weight = 0: WasteMethodName, WasteMaterialMixed, and WasteUnitOfMeasure columns are empty in CSV per METRC template semantics
- Created `src/lib/metrc-csv/generators/destroy-plants.ts` — #4 Destroy Plants CSV generator
  - Headers: Label,WasteMethodName,WasteMaterialMixed,WasteWeight,WasteUnitOfMeasureName,WasteReasonName,ReasonNote,ActualDate
  - Note: column name is WasteUnitOfMeasureName (not WasteUnitOfMeasure) — distinct from #3
  - One row per plant tag
- Updated `src/lib/metrc-csv/index.ts` to export both new generators
- Added two route handlers to `src/api/routes/metrc-csv.ts`:
  - POST /api/metrc/csv/destroy-immature (supervisor): validates count vs current batch count, generates CSV, INSERTs cv_metrc_immature_destruction_events, decrements plant_count_current, auto-closes batch when count reaches 0
  - POST /api/metrc/csv/destroy-plants (supervisor): validates all tags are active in cv_metrc_plant_state, generates CSV, marks each plant destroyed in cv_metrc_plant_state, unassigns cv_plant_assignments, upserts cv_plant_loss_events with METRC waste fields

### Key Decisions
- plant_count_current: cv_batches does not have this as a physical column (it's derived in batches routes from active assignments). The existing METRC CSV module routes reference it directly (split-planting, create-plantings do likewise). destroy-immature follows the same pattern, using batch.plant_count_initial as fallback if plant_count_current is NULL.
- Batch auto-close: when new count <= 0 after destruction, batch status → 'closed' and closed_date = actual_date; done atomically in the same transaction as the destruction event insert
- cv_plant_loss_events upsert: if an active assignment exists for the tag, checks for an existing loss event by plant_assignment_id; updates METRC fields if found, inserts a new record with loss_type='removal_culled', plant_disposition='disposed_waste' if not found. Tags with no active cv_plant_assignments record get a warning instead of failing the entire request.
- Phase 5 API: DELETE /plantbatches/v2/ for #3, DELETE /plants/v2/ for #4 (both confirmed)

### Files Modified/Created
- `src/lib/metrc-csv/generators/destroy-immature.ts` (new)
- `src/lib/metrc-csv/generators/destroy-plants.ts` (new)
- `src/lib/metrc-csv/index.ts` (two new exports added)
- `src/api/routes/metrc-csv.ts` (two Zod schemas + two route handlers added)

### Notes for Next Tasks
- `npx tsc --noEmit` passes clean; committed fd95656 and pushed to origin/master
- plant_count_current column gap: if the METRC CSV module needs reliable count tracking for immature batches, a migration to add plant_count_current as a real column to cv_batches (with default = plant_count_initial) would be needed. The current code handles NULL gracefully by falling back to plant_count_initial.
- Next METRC CSV batches: Phase 3C (Immature batch operations: #7 Growth Phase, #8 Packages, #9 Waste), Phase 3D (Plant lifecycle: #19 Growth Phase tag replace, #20 Location), Phase 3E (Harvest family: #5 Harvest Plants, #11 Manicure/Partial Harvest, #15 Packages from Harvest)

---

## Task: METRC CSV 3C: Immature batch generators #7 #8 #9
**Completed:** 2026-05-26T00:00:00Z

### What Was Done
- Created `src/lib/metrc-csv/generators/immature-growth-phase.ts` — #7 Immature Plants Growth Phase CSV generator
  - Headers: `Name,Count,StartingTag,GrowthPhase,NewLocation,NewSublocation,GrowthDate,PatientLicenseNumber`
  - Single-input generator (one batch graduation per request)
  - GrowthPhase: 'Vegetative' | 'Flowering'
- Created `src/lib/metrc-csv/generators/immature-packages.ts` — #8 Immature Plant Packages CSV generator
  - Headers: `PlantBatch,Item,Tag,PatientLicenseNumber,Note,IsTradeSample,IsDonation,Count,Location,Sublocation,ActualDate`
  - IsTradeSample/IsDonation rendered as 'True'/'False' (title case per METRC spec)
- Created `src/lib/metrc-csv/generators/immature-waste.ts` — #9 Immature Plants Waste CSV generator
  - Headers: `WasteMethodName,MixedMaterial,WasteWeight,UnitOfMeasureName,ReasonName,Note,WasteDate,PlantBatchName`
  - Multi-row batch endpoint (array of waste events)
- Updated `src/lib/metrc-csv/index.ts` with 6 new exports (generator functions + types for all three)
- Added three route handlers to `src/api/routes/metrc-csv.ts`:
  - POST /api/metrc/csv/immature-growth-phase (supervisor): validates batch exists, count ≤ current batch count, location exists in cv_locations, tag availability in cv_metrc_available_plant_tags. Transaction: INSERT cv_metrc_plant_state per tag (using WHERE tag >= starting_tag ORDER BY tag LIMIT count), UPDATE tags to 'used', decrement batch count, INSERT cv_metrc_csv_uploads.
  - POST /api/metrc/csv/immature-packages (supervisor): validates batch, item name, package tag availability. Transaction: INSERT cv_metrc_packages, UPDATE package tag to 'used', decrement batch count, INSERT cv_metrc_csv_uploads.
  - POST /api/metrc/csv/immature-waste (requireAuth grower+): resolves all batch names upfront via single IN query, validates all batch IDs exist. Transaction: INSERT cv_metrc_immature_waste_events per event, INSERT cv_metrc_csv_uploads.

### Key Decisions
- Tag range query for #7: `WHERE tag >= starting_tag AND status = 'available' ORDER BY tag LIMIT count` — relies on METRC sequential tag allocation (tags sort correctly as strings). Returns error if fewer available tags than requested count.
- Location lookup for #7: resolves cv_locations by metrc_name (text) → location_id (FK) for cv_metrc_plant_state insert.
- Package location for #8: stored as text (location_name) in cv_metrc_packages, not FK — matches existing schema. Warning logged if location not found but request proceeds.
- Batch name resolution for #9: single `SELECT batch_id, metrc_plant_batch_uid, name FROM cv_batches WHERE batch_id IN (?)` before building CSV rows — avoids N+1 queries.
- METRC API routes: #7 → POST /plantbatches/v2/growthphase, #8 → POST /plantbatches/v2/packages, #9 → POST /plantbatches/v2/waste (all Phase 5)

### Files Modified/Created
- `src/lib/metrc-csv/generators/immature-growth-phase.ts` (new)
- `src/lib/metrc-csv/generators/immature-packages.ts` (new)
- `src/lib/metrc-csv/generators/immature-waste.ts` (new)
- `src/lib/metrc-csv/index.ts` (6 new exports added)
- `src/api/routes/metrc-csv.ts` (4 new Zod schemas + 3 new route handlers added)
