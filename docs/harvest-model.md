...
|---|---|---|---|---|
| **Partial Harvest** | Product (wet weight) | Yes | During `harvesting`, multiple allowed per plant | "Manicure" |
| **Final Harvest** | Product (wet weight) | No — triggers unassignment + container teardown | During `harvesting`, once per plant | "Harvest" |
| **Waste Trim** | Waste (wet weight) | N/A — standalone event | Any stage: veg, flower, flush, harvesting | Plant waste |
...
| `harvest_batch` | Harvest (lot) | One METRC harvest lot per harvest_batch |
