...
export const api = {
  ...
  getProtocol: (id) => req('GET', `/tasks/protocols/${id}`),
  createProtocol: (data) => req('POST', '/tasks/protocols', data),
  updateProtocol: (id, data) => req('PATCH', `/tasks/protocols/${id}`, data),
  deleteProtocol: (id) => req('DELETE', `/tasks/protocols/${id}`),
  updateProtocolChecklist: (id, items) => req('PUT', `/tasks/protocols/${id}/checklist`, { items }),
  getChecklistProgress: (protocolId, batchId) => req('GET', `/tasks/checklist-progress?protocol_id=${protocolId}&batch_id=${batchId}`),
  saveChecklistProgress: (data) => req('PUT', '/tasks/checklist-progress', data),
  clearChecklistProgress: (protocolId, batchId) => req('DELETE', `/tasks/checklist-progress?protocol_id=${protocolId}&batch_id=${batchId}`),
  postponeTask: (data) => req('POST', '/tasks/postpone', data),
  resumeTask: (postponementId) => req('DELETE', `/tasks/postpone/${postponementId}`),
  getPostponements: (params = {}) => { const q = new URLSearchParams(params).toString(); return req('GET', `/tasks/postpone${q ? '?' + q : ''}`); },
  getSamplingContainerSuggestions: (batchId, count) => req('GET', `/tasks/sampling/suggest?batch_id=${batchId}&count=${count}`),
  createSamplingSession: (data) => req('POST', '/tasks/sampling', data),
  getSamplingSession: (id) => req('GET', `/tasks/sampling/${id}`),
  getSamplingSessions: (params = {}) => { const q = new URLSearchParams(params).toString(); return req('GET', `/tasks/sampling${q ? '?' + q : ''}`); },
  ...
  getMetrcTagAssignments: (params = {}) => { const q = new URLSearchParams(params).toString(); return req('GET', `/exports/metrc-tag-assignments${q ? '?' + q : ''}`); },
  ...
  getMetrcPhaseChanges: (params = {}) => { const q = new URLSearchParams(params).toString(); return req('GET', `/exports/metrc-phase-changes${q ? '?' + q : ''}`); },
  downloadMetrcPhaseChangesCsv: (params = {}) => { const q = new URLSearchParams({ ...params, format: 'csv' }).toString(); window.open(`/api/exports/metrc-phase-changes?${q}`); },
